...
export async function runAgentPhoneSession(agentId, rounds, {
  ...
  now = new Date(),
}: { engine?: any; signal?: any; conversationId?: any; conversationType?: string; systemAppend?: any; noMemory?: boolean; toolMode?: string; modelOverride?: any; onActivity?: any; onSessionReady?: any; emitEvents?: boolean; extraCustomTools?: any[]; returnDiagnostics?: boolean; cwd?: string | null; now?: Date } = {}) {
  ...
  const cwd = requestedCwd || engine.getHomeCwd(agentId) || process.cwd();
  if (requestedCwd) fs.mkdirSync(cwd, { recursive: true });
  ...
  const shouldOpenExistingSession = shouldReuseAgentPhoneSession({
    meta: runtime,
    sessionExists: existingSessionExists,
    now: refreshNow,
  });
  let openedExistingSession = shouldOpenExistingSession;
  let sessionManager: any = null;
  if (shouldOpenExistingSession && existingSessionPath) {
    const opened = SessionManager.open(existingSessionPath, sessionDir);
    const existingCwd = opened?.getCwd?.() || null;
    const sameRequestedCwd = !requestedCwd
      || (existingCwd && path.resolve(existingCwd) === path.resolve(cwd));
    if (sameRequestedCwd) {
      sessionManager = opened;
    } else {
      openedExistingSession = false;
