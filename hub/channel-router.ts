...
export class ChannelRouter {
  ...
  _resolveChannelWorkspaceRoot(channelName) {
    try {
      const filePath = path.join(this._engine.channelsDir, `${channelName}.md`);
      ...
      debugLog()?.warn?.("channel", `resolve channel workspace failed (#${channelName}): ${err.message}`);
      return null;
    }
  }
  ...
  async _executeReply(agentId, channelName, msgText, {
    ...
    const channelWorkspaceRoot = this._resolveChannelWorkspaceRoot(channelName);
    ...
    try {
      const phoneResult = await runAgentPhoneSession(
        ...
        ],
        {
          ...
          conversationId: channelName,
          conversationType: "channel",
          cwd: channelWorkspaceRoot || undefined,
          ...
          onSessionReady: (sessionPath) => {
            ...
            return this._recordPhoneActivity(
              ...
              isZh ? "正在查看手机群聊" : "Reading phone channel messages",
              {
                ...
                sessionPath,
                ...(channelWorkspaceRoot ? { workspaceRoot: channelWorkspaceRoot } : {}),
              },
            );
          },
