...
export class Hub {
  ...
  async send(text, opts: any = {}) {
    ...
    const o = { sessionKey, role, ephemeral, meta, isGroup, cwd, model, persist, permissionMode, from, to, onDelta, images, imageAttachmentPaths, videos, videoAttachmentPaths, audios, audioAttachmentPaths, inboundFiles, clientMessageId, sessionPath, agentId, uiContext, context, displayMessage, sessionFileRefs };

    // ── 图片预处理：持久化到磁盘 + 插入 [attached_image] 标记 ──
    // 在路由之前统一处理，所有消息路径（WS / Bridge DM / Bridge Group）共享
    if (
      o.images?.length
      && this._engine.openZetcXHome
      && !o.inboundFiles?.length
      && !hasDisplayImageAttachments(o.displayMessage)
    ) {
      const attachDir = path.join(this._engine.openZetcXHome, "attachments");
...
function publicAgentProfile(agent) {
  ...
  return {
    ...
    yuan: config.agent?.yuan || "openZetcX",
