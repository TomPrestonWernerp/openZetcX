...
description: Create openZetcX plugin scaffolds and guide users through beginner or developer plugin planning, capability checks, manifest setup, runtime tools, WebView/iframe UI, Session/Agent APIs, model/media APIs, SDK templates, and install-ready plugin directories. Use when openZetcX/Codex needs to explain what openZetcX plugins can do, help a user describe a plugin idea, check whether the SDK supports it, or generate/update a openZetcX plugin with @hana/plugin-runtime, @hana/plugin-sdk, and @hana/plugin-components.
...
Beginner mode tone: encouraging, concrete, and guided. Say that the user can describe the feature in plain language, openZetcX will help turn it into a plugin plan and scaffold, and openZetcX can answer questions at any step. Ask:
...
```bash
node skills2set/openZetcX-plugin-creator/scripts/check_env.mjs --capability scaffold
```
