...
description: Create openZetcX plugin scaffolds and guide users through beginner or developer plugin planning, capability checks, manifest setup, runtime tools, iframe UI, SDK templates, and install-ready plugin directories. Use when Codex needs to explain what openZetcX plugins can do, help a user describe a plugin idea, check whether the SDK supports it, or generate/update an openZetcX plugin with @hana/plugin-runtime, @hana/plugin-sdk, and @hana/plugin-components.
metadata:
  default-enabled: false
---

# openZetcX Plugin Creator

Use this skill for openZetcX application plugins, not Codex `.codex-plugin` bundles.

## First Contact

On first use, give a map, not an encyclopedia. Explain what openZetcX plugins can add, ask what the user wants to build, and invite follow-up questions. Expand details only after the user asks or after the chosen scaffold needs them.
...
Beginner mode tone: encouraging, concrete, and guided. Say that the user can describe the feature in plain language, openZetcX will help turn it into a plugin plan and scaffold, and the agent can answer questions at any step. Ask:

1. 你希望 OpenZetcX 多一个什么能力？
...
After delivering a plugin, encourage with grounded product value. Name the real situation where the plugin helps, such as reducing repeated steps, making an external service available inside openZetcX, turning a manual workflow into an Agent-callable tool, or giving a recurring task a stable UI. Use natural wording such as `这个想法挺实用，适合把每周重复整理的步骤固定下来` or `这个方向比较适合做成工具型插件，因为 Agent 可以在对话里直接调用`. Avoid inflated praise like `你的设想太棒了`.

## Capability Map

openZetcX plugins can provide:

- Agent-callable tools and slash-style actions.
- Skills, agents, and knowledge that guide model behavior.
- Iframe pages, widgets, and cards using openZetcX theme and host capabilities.
...
openZetcX provides install/enable/reload, per-agent skill toggles, manifest capability checks, iframe host messaging, theme tokens, toast/clipboard/external host APIs, EventBus, data directories, and SDK packages.
...
1. Find the openZetcX repo root. Prefer the current workspace if it contains `PLUGIN_SDK.md`, `PLUGINS.md`, and `packages/plugin-runtime`.
...
3. Pick a template:
   ...
   - `guided-react`: React/Vite/SDK starter with shared openZetcX plugin components and a gentler README.
...
5. Pick the target location:
   - Built-in plugin shipped with openZetcX: `plugins/<plugin-id>`.
...
9. If the user wants publication, choose one channel:
   ...
   - project marketplace release: prepare a `marketplace.json` entry and release zip, then run privacy-push before any remote push.
...
```bash
python3 skills2set/openZetcX-plugin-creator/scripts/create_openzetcx_plugin.py "My Plugin" --path examples/plugins --audience beginner --template direct
```

Developer React starter:

```bash
python3 skills2set/openZetcX-plugin-creator/scripts/create_openzetcx_plugin.py "My Plugin" --path examples/plugins --audience developer --template professional-react --sdk-mode workspace
```
...
```bash
python3 skills2set/openZetcX-plugin-creator/scripts/create_openzetcx_plugin.py "Jimeng Provider" --path examples/plugins --kind provider --audience developer
```
...
- Dev authority is not a manifest permission. openZetcX grants it from the remembered dev install slot under `${HANA_HOME}/plugins-dev/`, and Agent dev tools are hidden until the user enables the dev tools setting.
...
- Marketplace metadata lives in this project's `marketplace.json`.
- Source plugins may live in this repository with matching `marketplace.json` entries.
...
- For a single release, root `version`, `compatibility`, and `distribution` remain valid; openZetcX normalizes them into a single version entry.
- openZetcX selects the highest SemVer version compatible with the current app and exposes update, reinstall, incompatible, and downgrade states to the UI.
...
- Before pushing marketplace changes, run privacy-push and wait for explicit user confirmation.

## UI Rules

- Default React plugin UI to `HanaThemeProvider mode="inherit"` so it follows the host theme.
- Use `mode="hana"` for a named openZetcX theme, and `mode="custom"` only for explicit token overrides.
