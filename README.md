...
目前的openZetc还是只是一个本地化的agent工具，它只是将龙虾、hermes等一些agent工具做了本地化集成，该项目的最初目的是降低普通人使用龙虾的门槛，后面为个人探索多agent协同交互的学习内容；本人为后端开发人员，当前项目需求正在持续扩大，已经超过了一个本地工具的个人应用范畴；后续开发维护可能不会特别及时；
## 功能特性

**记忆** — 结合主流的记忆方案，自己又发挥了一下，做了个记忆系统，近期的事情记得非常牢固，但目前确实有待优化。

**集群** — 集群可以让多个职业、能力各不相同的agent在同一环境下协作运行处理给他们所安排的事物，多个agent之间可以相互交流、沟通；你可以在集群内与多个agent进行头脑风暴、任务执行等；
...
> 仓库和 release 地址在当前迁移阶段仍保留旧的 `openZetcX` URL，后续仓库 rename 会单独执行。

- [官网](https://openZetcX.com)
...
- [国内 AtomGit 托管](https://gitcode.com/TomPrestonWernerp/openZetcX)
