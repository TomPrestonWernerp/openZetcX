/**
 * openZetcX Server — HTTP + WebSocket API
 *
 * 启动方式：
...
// 用户数据存放在 ~/.openZetcX/（打包后与产品代码分离）
// 开发时可通过 HANA_HOME 环境变量隔离数据目录，如：HANA_HOME=~/.openZetcX-dev node server/index.js
...
const serverNetwork = resolveServerListenOptions(openZetcXHome);
...
const firstRunReport = ensureFirstRun(openZetcXHome, productDir);
...
ensureLocalIdentityRegistries(openZetcXHome);
log.log("① ensureLocalIdentityRegistries 完成");

// ── 初始化 Debug 日志 ──
const dlog = initDebugLog(path.join(openZetcXHome, "logs"));

// ── 初始化引擎 ──
log.log("② 创建 HanaEngine...");
const engine: any = new HanaEngine({ openZetcXHome, productDir, appVersion } as any);
...
BrowserManager.setOpenZetcXHome(engine.openZetcXHome);
...
const deferredResultStore = new DeferredResultStore(
  hub.eventBus,
  path.join(openZetcXHome, ".ephemeral", "deferred-tasks.json"),
...
const workflowActivityStore = new WorkflowActivityStore(
  path.join(openZetcXHome, "workflow-activity.json"),
);
...
app.get("/api/health", async (c) => {
  ...
  return c.json({
    ...
    agent: engine.agentName,
    agentYuan: engine.agent?.config?.agent?.yuan || "openZetcX",
...
try {
  ...
  browserWss.on("connection", (ws) => {
    ...
    const _bwsLogPath = path.join(openZetcXHome, "browser-ws.log");
  ...
  log.log(`openZetcX Server 运行在 http://${host}:${actualPort}`);
