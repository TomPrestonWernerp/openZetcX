  ...
  route.post("/sessions/switch", async (c) => {
    try {
      ...
      try {
        ...
        return c.json({
          error: t("error.sessionNotFound"),
          staleSession: true,
        });
      }
      const targetAgentId = engine.agentIdFromSessionPath?.(sessionPath) || null;
      if (targetAgentId && engine.getAgent?.(targetAgentId) == null) {
        return c.json({
          error: t("error.agentNotFound", { id: targetAgentId }),
          staleSession: true,
          missingAgentId: targetAgentId,
        });
      }
      ...
      const switchedAgentId = targetAgentId || engine.currentAgentId;
      ...
      try { appendFileSync(path.join(engine.openZetcXHome, "switch-error.log"), `${new Date().toISOString()}\n${errDetail}\n---\n`); } catch {}
      return c.json({ error: err.message }, 500);
    }
  });
  ...
  route.post("/sessions/archive", async (c) => {
    try {
      ...
      try {
        ...
        return c.json({ ok: true, missing: true });
      }
...
function patchSessionFileLifecycleBlocks(blocks, engine, sessionPath) {
  ...
  for (const block of blocks || []) {
    ...
    if (!file && block.type === "screenshot" && block.base64 && engine?.openZetcXHome && typeof engine?.getSessionFileByPath === "function") {
      try {
        const filePath = browserScreenshotPath(engine.openZetcXHome, sessionPath, {
          base64: block.base64,
