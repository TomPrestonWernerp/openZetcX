...
export function createServerIdentityRoute({ openZetcXHome, appVersion = "?", getRuntimeContext }: { openZetcXHome?: string; appVersion?: string; getRuntimeContext?: () => any } = {}) {
  ...
  route.get("/server/identity", (c) => {
    try {
      const runtimeContext = typeof getRuntimeContext === "function"
        ? getRuntimeContext()
        : createServerRuntimeContext({ openZetcXHome, appVersion });
