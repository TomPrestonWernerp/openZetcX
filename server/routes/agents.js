...
function firstExisting(paths) {
  return paths.find((p) => p && fsSync.existsSync(p));
}

function fillTemplate(content, { agentName, userName }) {
  return content
    .replace(/\{\{agentName\}\}/g, agentName)
    .replace(/\{\{userName\}\}/g, userName);
}

async function applyYuanTemplatesForAgent(engine, id, config) {
  if (!engine.productDir) return;

  const yuanType = config.agent?.yuan || "openZetcX";
  const agentName = config.agent?.name || engine.getAgent?.(id)?.agentName || id;
  const userName = config.user?.name || engine.getAgent?.(id)?.userName || "User";
  const isZh = String(config.locale || engine.getLocale?.() || "zh").startsWith("zh");
  const langDir = isZh ? "" : "en/";
  const dir = agentDir(engine, id);

  const identitySrc = firstExisting([
    path.join(engine.productDir, "identity-templates", `${langDir}${yuanType}.md`),
    path.join(engine.productDir, "identity-templates", `${yuanType}.md`),
    path.join(engine.productDir, "identity.example.md"),
  ]);
  if (identitySrc) {
    const tmpl = await fs.readFile(identitySrc, "utf-8");
    await fs.writeFile(
      path.join(dir, "identity.md"),
      fillTemplate(tmpl, { agentName, userName }),
      "utf-8",
    );
  }

  const ishikiSrc = firstExisting([
    path.join(engine.productDir, "ishiki-templates", `${langDir}${yuanType}.md`),
    path.join(engine.productDir, "ishiki-templates", `${yuanType}.md`),
    path.join(engine.productDir, "ishiki.example.md"),
  ]);
  if (ishikiSrc) {
    await fs.copyFile(ishikiSrc, path.join(dir, "ishiki.md"));
  }

  const publicIshikiSrc = firstExisting([
    path.join(engine.productDir, "public-ishiki-templates", `${langDir}${yuanType}.md`),
    path.join(engine.productDir, "public-ishiki-templates", `${yuanType}.md`),
  ]);
  if (publicIshikiSrc) {
    await fs.copyFile(publicIshikiSrc, path.join(dir, "public-ishiki.md"));
  }
}
...
export function createAgentsRoute(engine) {
  ...
  route.put("/agents/:id/config", async (c) => {
    ...
    try {
      ...
      const configPath = path.join(agentDir(engine, id), "config.yaml");
      const existingConfig = YAML.load(fsSync.readFileSync(configPath, "utf-8")) || {};
      ...
      if (hasOwn(agentPartial?.agent, "yuan")) {
        ...
        const locale = getGlobalValue(globalFields, "locale");
        if (locale !== undefined) effectiveConfig.locale = locale;
        await applyYuanTemplatesForAgent(engine, id, effectiveConfig);
      }
