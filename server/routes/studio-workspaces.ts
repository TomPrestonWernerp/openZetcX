...
async function listStudioWorkspaces(engine, requestContext) {
  ...
  for (const mount of listStudioMountsForStudio(engine.openZetcXHome, studioId)) {
...
async function createLocalPathWorkspace(engine, requestContext, body) {
  ...
  const mount = upsertStudioMount(engine.openZetcXHome, {
...
function disableLocalPathWorkspace(engine, requestContext, mountId) {
  const studioId = requireStudioId(requestContext);
  const activeMount = listStudioMountsForStudio(engine.openZetcXHome, studioId)
  ...
  return disableStudioMount(engine.openZetcXHome, mountId, { hostStudioId: studioId });
}
