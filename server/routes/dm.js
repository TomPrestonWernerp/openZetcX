...
export function createDmRoute(engine) {
  ...
  function getAgentDisplayName(agent, fallback) {
    return agent?.agentName || agent?.name || agent?.config?.agent?.name || fallback;
  }

  function ensureDmFile(agent, peerId, peerName) {
    ...
    if (!fs.existsSync(dmFile)) {
      ...
      const content = [
        "---",
        `id: dm:${peerId}`,
        `members: [${agent.id}, ${peerId}]`,
        `peer: ${peerId}`,
        `name: ${peerName}`,
        "---",
        "",
      ].join("\n");
      fs.writeFileSync(dmFile, content, "utf-8");
    }
    return dmFile;
  }
  ...
  route.get("/dm/:peerId", async (c) => {
    try {
      ...
      const peerName = getAgentDisplayName(peerAgent, peerId);
      const dmFile = ensureDmFile(agent, peerId, peerName);
