...
import { WORKSPACE_OUTPUT_ROOT_DIRNAME, resolveWorkspaceOutputRoot } from "../../shared/workspace-output.ts";
...
function isInsideAnyRoot(dir, roots) {
  const resolved = realPath(dir) || comparablePath(dir);
  if (!resolved) return false;
  return roots.filter(Boolean).some(root => {
    const r = realPath(root) || comparablePath(root);
    if (!r) return false;
...
function comparablePath(value) {
  ...
  const parent = realPath(path.dirname(value));
...
function engineWorkspaceParentRoots(engine) {
...
function isWorkspaceOutputRelativePath(value) {
  const normalized = value.replace(/\\/g, "/");
  return normalized === WORKSPACE_OUTPUT_ROOT_DIRNAME
    || normalized.startsWith(`${WORKSPACE_OUTPUT_ROOT_DIRNAME}/`);
}

function resolveRelativeChannelWorkspaceRoots(value, engine, mode = "channel") {
  if (typeof value !== "string" || !value.trim()) return [];
  const relative = value.trim().replace(/^[\\/]+|[\\/]+$/g, "");
  if (!relative || relative === "." || relative.split(/[\\/]+/).includes("..")) return [];
  const normalizedRelative = relative.replace(/[\\/]+/g, path.sep);
  const roots = [];
  for (const parentRoot of engineWorkspaceParentRoots(engine)) {
    try {
      const baseRoot = mode === "base" || isWorkspaceOutputRelativePath(relative)
        ? parentRoot
        : resolveWorkspaceOutputRoot(parentRoot);
      roots.push(path.resolve(baseRoot, normalizedRelative));
    } catch {
      // Ignore roots that cannot form a workspace output directory.
    }
  }
  return uniquePathList(roots);
}

function resolveConfiguredChannelWorkspaceRoots(value, engine, mode = "channel") {
  if (typeof value !== "string" || !value.trim()) return [];
  const trimmed = value.trim();
  if (isAbsoluteWorkspacePath(trimmed)) return [trimmed];
  return resolveRelativeChannelWorkspaceRoots(trimmed, engine, mode);
}

function resolveDeskDirOverride(value, engine) {
  if (typeof value !== "string" || !value.trim()) return value;
  const trimmed = value.trim();
  if (isAbsoluteWorkspacePath(trimmed)) return trimmed;
  const candidates = resolveRelativeChannelWorkspaceRoots(trimmed, engine, "channel");
  return candidates.find(candidate => fs.existsSync(candidate)) || candidates[0] || trimmed;
}
...
function readChannelWorkspaceRoot(filePath) {
  try {
    const content = fs.readFileSync(filePath, "utf-8");
    const match = content.match(/^---\s*\r?\n([\s\S]*?)\r?\n---/);
    const frontmatter = match?.[1] || "";
    for (const line of frontmatter.split(/\r?\n/)) {
      const field = line.match(/^(?:workspaceRoot|workspace_root)\s*:\s*(.*)$/);
      if (!field) continue;
...
function configuredChannelWorkspaceRoots(engine) {
  const roots = [];
  const pushRoots = (value, mode = "channel") => {
    roots.push(...resolveConfiguredChannelWorkspaceRoots(value, engine, mode));
  };
  const channelsDir = engine?.channelsDir;
  if (typeof channelsDir === "string" && fs.existsSync(channelsDir)) {
    try {
      for (const fileName of fs.readdirSync(channelsDir)) {
        if (!fileName.endsWith(".md")) continue;
        const root = readChannelWorkspaceRoot(path.join(channelsDir, fileName));
        if (root) pushRoots(root, "channel");
      }
    } catch {
      // Ignore malformed channel metadata here; normal desk approval still applies.
    }
  }

  const globalRoot = engine?.config?.channel?.workspace_root;
  if (typeof globalRoot === "string" && globalRoot.trim()) {
    pushRoots(globalRoot, "base");
  }

  const configured = engine?.config?.channel?.workspaces;
  if (configured && typeof configured === "object") {
    for (const value of Object.values(configured)) {
      if (value && typeof value === "object") {
        const root = (value as any).workspace_root || (value as any).workspaceRoot;
        if (typeof root === "string" && root.trim()) pushRoots(root, "channel");
      }
    }
  }
  return uniquePathList(roots);
...
function defaultChannelWorkspaceRoots(engine) {
  const roots = [
    engine?.defaultDeskCwd,
    engine?.homeCwd,
    engine?.deskCwd,
  ...
  return roots.map(root => {
    try {
      return resolveWorkspaceOutputRoot(root);
    } catch {
      return null;
    }
  }).filter(Boolean);
...
function isApprovedDir(dir, engine, { agentId = null } = {}) {
  ...
  if (isInsideAnyRoot(dir, configuredChannelWorkspaceRoots(engine))) {
    return true;
  }
  if (isInsideAnyRoot(dir, defaultChannelWorkspaceRoots(engine))) {
    return true;
  }
...
const MAX_DESK_UPLOAD_BYTES = 25 * 1024 * 1024;
...
export function createDeskRoute(engine, hub) {
  ...
  function resolveWorkspaceRootFromRequest(c, body = null) {
    ...
    const requestedDir = body?.dir || (c.req.query("dir") ? decodeURIComponent(c.req.query("dir")) : null);
    const dir = requestedDir ? resolveDeskDirOverride(requestedDir, engine) : defaultDeskDir(engine);
  ...
  function writeUploadedCoverImage(body) {
    ...
    const uploadRoot = path.join(engine.openZetcXHome || os.tmpdir(), "tmp", "markdown-cover-uploads");
  ...
  route.get("/desk/path", async (c) => {
    const agentId = c.req.query("agentId") || null;
    const dir = c.req.query("dir") ? resolveDeskDirOverride(decodeURIComponent(c.req.query("dir")), engine) : defaultDeskDir(engine);
  ...
  route.post("/desk/jian", async (c) => {
    const body = await safeJson(c);
    const agentId = body.agentId || null;
    const dir = body.dir ? resolveDeskDirOverride(body.dir, engine) : defaultDeskDir(engine);
  ...
  route.post("/desk/files", async (c) => {
    const body = await safeJson(c);
    const agentId = body.agentId || null;
    const baseDir = body.dir ? resolveDeskDirOverride(body.dir, engine) : defaultDeskDir(engine);
    ...
    switch (action) {
      case "upload": {
        ...
        if (uploadedFiles.length > 0) {
          ...
          for (const file of uploadedFiles) {
            try {
              if (!file || typeof file !== "object" || !isPlainEntryName(file.name)) {
                throw new Error("invalid name");
              }
              if (typeof file.contentBase64 !== "string") {
                throw new Error("contentBase64 required");
              }
              const contentBase64 = file.contentBase64.replace(/^data:[^,]*,/, "");
              const buffer = Buffer.from(contentBase64, "base64");
              if (buffer.byteLength > MAX_DESK_UPLOAD_BYTES) {
                return c.json({ error: "file too large" }, 413);
              }
              const written = await files.writeFileContent("default", subdirStr, file.name, buffer, { reason: reasonFor("upload") });
              results.push({ name: written.filename, ok: true, size: buffer.byteLength });
            } catch (err) {
              results.push({ name: file?.name || null, ok: false, error: err?.message || "upload_failed" });
            }
          }
          return c.json({
            ok: results.every((item) => item.ok),
            results,
            files: await files.filesForDirectory("default", subdirStr),
          });
        }
        ...
        for (const srcPath of paths) {
          try {
            ...
            if (isSensitivePath(srcPath, engine.openZetcXHome)) {
      ...
      case "mkdir": {
        ...
        if (!isPlainEntryName(name)) return c.json({ error: "invalid name" });
        const targetRel = subdirStr ? `${subdirStr}/${name}` : name;
        const targetDir = subdirToDir(targetRel);
        if (targetDir && fs.existsSync(targetDir) && fs.statSync(targetDir).isDirectory()) {
          const files = fileServiceForRequest(c, baseDir);
          return c.json({ ok: true, files: await files.filesForDirectory("default", subdirStr) });
        }
        const files = fileServiceForRequest(c, baseDir);
