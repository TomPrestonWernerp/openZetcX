...
function normalizeChannelWorkspaceRoot(value: any) {
  if (value === undefined || value === null || value === "") return "";
  if (typeof value !== "string") {
    const err: any = new Error("workspaceRoot must be a string");
    err.status = 400;
    throw err;
  }
  const root = value.trim();
  if (!root || /[\x00\r\n]/.test(root)) {
    const err: any = new Error("workspaceRoot must be a non-empty path");
    err.status = 400;
    throw err;
  }
  return root;
...
export function createChannelsRoute(engine: any, hub: any) {
    ...
    try {
      ...
      for (const f of files) {
        ...
        channels.push({
          ...
          members,
          workspaceRoot: typeof meta.workspaceRoot === "string" && meta.workspaceRoot.trim()
            ? meta.workspaceRoot.trim()
            : null,
  ...
  route.post("/channels", async (c) => {
    try {
      ...
      try {
        workspaceRoot = body.workspaceRoot === undefined || body.workspaceRoot === null || body.workspaceRoot === ""
          ? undefined
          : normalizeChannelWorkspaceRoot(body.workspaceRoot);
      } catch (err) {
      ...
      return c.json({ ok: true, id: channelId, name, members: normalizedMembers, workspaceRoot: workspaceRoot || null });
    } catch (err) {
  ...
  // ── 添加频道成员 ──
  route.post("/channels/:name/workspace", async (c) => {
    try {
      const disabled = requirePhoneEnabled(c);
      if (disabled) return disabled;
      const name = c.req.param("name");
      ...
      const body = await safeJson(c);
      const workspaceRoot = normalizeChannelWorkspaceRoot(body.workspaceRoot);
      await updateChannelMeta(filePath, { workspaceRoot });
      return c.json({ ok: true, workspaceRoot });
    } catch (err) {
      return c.json({ error: err.message }, err.status || 500);
    }
  });
