...
import { assertAgentConfigPatchYuan, normalizeAgentConfigYuanInPlace } from "../../core/yuan-registry.ts";
...
function resolveRolePresetAvatarPath(engine, rolePreset) {
  const preset = getOpenZetcXRolePreset(rolePreset);
  if (!preset) return undefined;
  const avatarPath = path.join(engine.productDir, "role-avatars", preset.avatarFile);
  return fsSync.existsSync(avatarPath) ? avatarPath : undefined;
}

const AGENT_TEMPLATE_PLACEHOLDER_RE = /\{\{(?:userName|agentName|agentId)\}\}/;

function hasAgentTemplatePlaceholders(content) {
  return AGENT_TEMPLATE_PLACEHOLDER_RE.test(content || "");
}

function renderAgentTemplatePlaceholders(content, config, agentId) {
...
async function readRenderedAgentTemplateFile(engine, id, fileName) {
  ...
  let cfg = {};
  try {
    cfg = YAML.load(await fs.readFile(path.join(dir, "config.yaml"), "utf-8")) || {};
  } catch {}
...
export function createAgentsRoute(engine) {
  ...
  route.post("/agents", async (c) => {
    try {
      ...
      const { name, id, yuan, rolePreset } = body;
      if (!name?.trim()) {
  ...
  route.get("/agents/:id/config", async (c) => {
    ...
    try {
      ...
      normalizeAgentConfigYuanInPlace(config);
  ...
  route.get("/agents/:id/identity", async (c) => {
    ...
    try {
      const content = await readRenderedAgentTemplateFile(engine, id, "identity.md");
  ...
  route.get("/agents/:id/public-ishiki", async (c) => {
    ...
    try {
      const content = await readRenderedAgentTemplateFile(engine, id, "public-ishiki.md");
      return c.json({ content });
    } catch (err) {
      if (err.code === "ENOENT") return c.json({ content: "" });
      return c.json({ error: err.message }, 500);
