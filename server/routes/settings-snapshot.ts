...
import { normalizeAgentConfigYuanInPlace } from "../../core/yuan-registry.ts";
...
function hasAgentTemplatePlaceholders(content: string) {
  return AGENT_TEMPLATE_PLACEHOLDER_RE.test(content || "");
}

function renderAgentTemplatePlaceholders(content: string, config: Record<string, any>, agentId: string) {
...
async function readRenderedAgentTemplateFile(filePath: string, config: Record<string, any>, agentId: string) {
...
function buildAccessSnapshot(engine: any, canSeeLocalPaths: boolean, options: Record<string, any>) {
  if (!canSeeLocalPaths || !engine?.openZetcXHome) return null;
  ...
  route.get("/settings/snapshot", async (c) => {
    try {
      ...
      const [identity, ishiki, publicIshiki] = await Promise.all([
        readRenderedAgentTemplateFile(path.join(baseDir, "identity.md"), config, agentId),
        readTextFile(path.join(baseDir, "ishiki.md")),
        readRenderedAgentTemplateFile(path.join(baseDir, "public-ishiki.md"), config, agentId),
      ]);
