...
export function createWebAuthRoute({
  ...
  route.post("/web-auth/login", async (c) => {
    ...
    const issued = createWebSession(openZetcXHome, {
      principal,
  ...
  route.post("/web-auth/logout", async (c) => {
    revokeWebSession(openZetcXHome, c.req.header("cookie"), { now: now() });
...
function authenticatePasswordLogin(c, {
  ...
  const verified = verifyLocalAccountPassword(openZetcXHome, { username, password });
