...
export function createAccessRoute({
  ...
  if (!engine?.openZetcXHome) throw new Error("engine.openZetcXHome required");
  ...
  route.put("/access/network", async (c) => {
    ...
    try {
      ...
      const existing = loadServerNetworkConfig(engine.openZetcXHome);
      ...
      const network = saveServerNetworkConfig(engine.openZetcXHome, {
        ...existing,
        mode,
        listenHost,
        listenPort,
  ...
  async function issueAccessCredential(c, profile) {
    ...
    try {
      ...
      const issued = createDeviceCredential(engine.openZetcXHome, {
  ...
  route.put("/access/account/profile", async (c) => {
    ...
    try {
      const body = await safeJson(c);
      const account = updateLocalAccountProfile(engine.openZetcXHome, {
  ...
  route.put("/access/account/password", async (c) => {
    ...
    try {
      const body = await safeJson(c);
      const account = setLocalAccountPassword(engine.openZetcXHome, {
  ...
  route.delete("/access/account/password", async (c) => {
    ...
    if (denied) return denied;
    try {
      const account = clearLocalAccountPassword(engine.openZetcXHome, { now: now() });
      recordSecurityAuditEvent(c, engine, {
...
export function createAccessSummary(engine, runtimeState, listLanAddresses = getLanAddresses) {
  const network = loadServerNetworkConfig(engine.openZetcXHome);
  const registries = loadDeviceAccessRegistries(engine.openZetcXHome);
  return {
    network: createNetworkSummary(network, runtimeState, listLanAddresses),
    account: getLocalAccountSummary(engine.openZetcXHome),
