...
describe("macos Cua provider", () => {
  ...
  it("prefers a Hana-bundled Computer Use helper over an external Cua Driver install", () => {
    const command = resolveCuaDriverCommand({
      env: {
        HANA_ROOT: "/Applications/openZetcX.app/Contents/Resources/server",
        HANA_CUA_DRIVER_PATH: "/opt/cua-driver",
      },
      existsSync: (p) => p === "/Applications/openZetcX.app/Contents/Resources/computer-use/macos/hana-computer-use-helper"
        || p === "/opt/cua-driver",
      homeDir: "/Users/hana",
      arch: "arm64",
    ...
    expect(command).toBe("/Applications/openZetcX.app/Contents/Resources/computer-use/macos/hana-computer-use-helper");
  });
