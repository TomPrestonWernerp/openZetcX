...
async function makeRouteResourceIO({ openZetcXHome, workspace, eventBus = {}, studioId = "studio_1" }: Record<string, any>) {
...
describe("mobile workbench route", () => {
  ...
  it("lists workbench files without exposing absolute server paths", async () => {
    ...
    const app = await makeApp({
      openZetcXHome: path.join(tmpDir, "hana"),
      deskCwd: workspace,
      homeCwd: workspace,
    });
  ...
  it("discloses local_fs native roots to the local owner principal only", async () => {
    ...
    const openZetcXHome = path.join(tmpDir, "hana");
    fs.mkdirSync(workspace, { recursive: true });
  ...
  it("safe-deletes mobile files into recoverable trash instead of hard removing bytes", async () => {
    ...
    const trashDir = path.join(openZetcXHome, "trash", "mobile-workbench", data.trashId);
  ...
  it("creates and consumes an execution lease for remote mobile writes", async () => {
    ...
    const leases = JSON.parse(fs.readFileSync(path.join(openZetcXHome, "security", "execution-leases.json"), "utf-8"));
    ...
    const audit = fs.readFileSync(path.join(openZetcXHome, "logs", "security-audit.jsonl"), "utf-8");
