...
describe("character-card import service", () => {
  ...
  it("creates a suffixed public skill copy when a packaged skill name already exists", async () => {
    ...
    expect(plan.assets).not.toHaveProperty("cardBack");
  ...
  it("plans and commits through the route, then emits agent and skill events", async () => {
    ...
    expect(planData.plan).toMatchObject({
      packageName: "package",
      agent: { name: "Research Hana", yuan: "openZetcX" },
      skills: { count: 1 },
    });
  ...
  it("exports the selected agent as a card package with enabled skills and optional memory", async () => {
    ...
    expect(preview).toMatchObject({
      ...
      agent: { name: "Hana", yuan: "openZetcX", description: "花名册描述" },
      memory: {
        available: true,
        count: 5,
        preview: "用户喜欢短句。",
    ...
    expect(preview.assets).not.toHaveProperty("cardBack");
    ...
    expect(card.agent).toEqual({ name: "Hana", yuan: "openZetcX", description: "花名册描述" });
    ...
    expect(fs.existsSync(path.join(outDir, "assets/card-back.png"))).toBe(false);
    expect(card.assets).not.toHaveProperty("cardBack");
  });
