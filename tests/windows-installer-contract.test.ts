...
describe("Windows NSIS installer contract", () => {
  it("does not let stale old-uninstaller failures abort a openZetcX-owned overlay", () => {
  ...
  it("future uninstallers remove openZetcX-owned install surfaces without atomic old-install staging", () => {
    const source = fs.readFileSync(path.join(root, "build", "installer.nsh"), "utf-8");
  ...
  it("overrides app-running detection to close openZetcX, legacy Hanako, and the bundled server explicitly", () => {
    const source = fs.readFileSync(path.join(root, "build", "installer.nsh"), "utf-8");
  ...
  it("pins the Windows executable name to the current product identity", () => {
    ...
    expect(pkg.build.win.executableName).toBe("openZetcX");
    expect(pkg.build.nsis.shortcutName).toBe("openZetcX");
  });
