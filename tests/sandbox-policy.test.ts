...
describe("sandbox workspace roots", () => {
  ...
  it("lets agents read session files but blocks writing runtime copies", () => {
    ...
    const sessionFile = path.join(openZetcXHome, "session-files", "abc123", "SKILL.md");
    ...
    expect(policy.writablePaths).not.toContain(path.join(openZetcXHome, "session-files"));
  ...
  it("honors read-all semantics for non-secret HANA_HOME paths while keeping writes scoped", () => {
    ...
    const openZetcXHome = path.join(tempRoot, "home");
    const workspace = path.join(tempRoot, "project");
    const pluginSkill = path.join(openZetcXHome, "plugins", "demo", "skills", "reader", "SKILL.md");
    const pluginSource = path.join(openZetcXHome, "plugins", "demo", "index.js");
    const blockedAuth = path.join(openZetcXHome, "auth.json");
  ...
  it("treats cwd and explicit runtime roots as scoped write roots", () => {
    ...
    for (const dir of [agentDir, openZetcXHome, workspace, cwd, runtimeRoot]) {
      fs.mkdirSync(dir, { recursive: true });
    }
