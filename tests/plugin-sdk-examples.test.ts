...
const bundledSdkDir = path.join(root, "skills2set", "openZetcX-plugin-creator", "assets", "sdk");
...
describe("plugin SDK examples and docs", () => {
  it("uses an absolute file URL for workspace SDK dependencies across Windows drives", () => {
    const scriptPath = path.join(root, "skills2set", "openZetcX-plugin-creator", "scripts", "create_openzetcx_plugin.py");
    const result = execFileSync("python3", [
      ...
      [
        ...
        "import sys",
        "spec = importlib.util.spec_from_file_location('create_openzetcx_plugin', sys.argv[1])",
        "mod = importlib.util.module_from_spec(spec)",
        "spec.loader.exec_module(mod)",
        "print(mod.relative_file_spec(",
        "    PureWindowsPath('C:/Users/runner/AppData/Local/Temp/hana-ui-scaffold/sdk-panel'),",
        "    PureWindowsPath('D:/a/openZetcX/openZetcX/packages/plugin-sdk'),",
        "))",
      ].join("\n"),
      scriptPath,
    ], { cwd: root, encoding: "utf-8" }).trim();

    expect(result).toBe("file:///D:/a/openZetcX/openZetcX/packages/plugin-sdk");
  });
  ...
  it("documents ResourceIO access for user resources in plugin guides and Hana Plugin Creator", () => {
    ...
    const creatorSkill = fs.readFileSync(path.join(root, "skills2set", "openZetcX-plugin-creator", "SKILL.md"), "utf-8");
  ...
  it("keeps openZetcX-plugin-creator bundled SDK tarballs aligned with current runtime and protocol APIs", () => {
