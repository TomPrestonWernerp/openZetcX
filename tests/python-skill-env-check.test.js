...
const pluginCreatorCheck = path.join(repoRoot, "skills2set", "openZetcX-plugin-creator", "scripts", "check_env.mjs");
...
describe("openZetcX-plugin-creator environment check", () => {
  const roots = [];
  ...
  it("reports missing Python before running plugin scaffold scripts", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "openZetcX-plugin-creator-env-"));
    roots.push(root);
