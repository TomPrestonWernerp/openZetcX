...
describe("Agent platform prompt identity", () => {
  it("describes the current openZetcX platform name and the former openZetcX name in Chinese", () => {
    const prompt = makeAgent("zh-CN").buildSystemPrompt({
      forceMemoryEnabled: false,
      forceExperienceEnabled: false,
    });

    expect(prompt).toContain("你运行在 openZetcX 平台上（原名 openZetcX）");
    expect(prompt).toContain("https://github.com/TomPrestonWernerp/openZetcX");
  });

  it("describes the current openZetcX platform name and the former openZetcX name in English", () => {
    const prompt = makeAgent("en").buildSystemPrompt({
      forceMemoryEnabled: false,
      forceExperienceEnabled: false,
    });

    expect(prompt).toContain("You are running on the openZetcX platform (formerly openZetcX)");
    expect(prompt).toContain("https://github.com/TomPrestonWernerp/openZetcX");
  });
