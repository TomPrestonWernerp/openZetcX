...
describe("upload route", () => {
  ...
  it("rejects a symlink root path", async () => {
    ...
    const app = new Hono();
    app.route("/api", createUploadRoute({ openZetcXHome: path.join(tmpDir, "hana-home") }));
  ...
  it("upload-blob writes base64 image to uploads dir with sanitized name", async () => {
    tmpDir = mktemp();
    const openZetcXHome = path.join(tmpDir, "hana-home");
    const app = new Hono();
    app.route("/api", createUploadRoute({ openZetcXHome }));
  ...
  it("registers copied uploads as session files when sessionPath is provided", async () => {
    ...
    const app = new Hono();
    app.route("/api", createUploadRoute({ openZetcXHome, registerSessionFile }));
    ...
    expect(data.uploads[0].dest.startsWith(path.join(openZetcXHome, "session-files"))).toBe(true);
    expect(data.uploads[0]).toMatchObject({
      src: source,
      name: "note.txt",
      fileId: "sf_upload",
  ...
  it("reuses one session file for repeated uploads of the same source file", async () => {
    ...
    const registry = new SessionFileRegistry({ managedCacheRoot: path.join(openZetcXHome, "session-files") });
    const engine = {
      openZetcXHome,
  ...
  it("upload-blob forces extension to match mimeType (defends against name spoofing)", async () => {
    ...
    // 确保落点在 uploads 目录内
    expect(up.dest.startsWith(path.join(openZetcXHome, "uploads"))).toBe(true);
  });
