...
describe("sessions route", () => {
  ...
  it("restores browser state for the target session after switch", async () => {
    ...
    const oldPath = path.join(agentsDir, "a", "sessions", "old.jsonl");
    const newPath = path.join(agentsDir, "a", "sessions", "new.jsonl");
    fs.mkdirSync(path.dirname(newPath), { recursive: true });
    fs.writeFileSync(oldPath, "{}\n", "utf-8");
    fs.writeFileSync(newPath, "{}\n", "utf-8");
    browserManagerMock._sessions.set(oldPath, { running: true, url: "https://before.example.com" });

    const engine = {
      agentsDir,
      currentSessionPath: oldPath,
    ...
    const res = await app.request("/api/sessions/switch", {
      ...
      body: JSON.stringify({ path: newPath, currentSessionPath: oldPath }),
    });

    const data = await res.json();
    expect(res.status).toBe(200);
    expect(browserManagerMock.suspendForSession).toHaveBeenCalledWith(oldPath);
    expect(browserManagerMock.resumeForSession).toHaveBeenCalledWith(newPath);
  ...
  it("returns a stale-session response when switching to a deleted agent session", async () => {
    const { createSessionsRoute } = await import("../server/routes/sessions.js");
    const app = new Hono();
    const agentsDir = path.join(tmpDir, "agents");
    const sessionPath = path.join(agentsDir, "xiaoming", "sessions", "old.jsonl");
    ...
    const engine = {
      ...
      agentIdFromSessionPath: vi.fn(() => "xiaoming"),
      getAgent: vi.fn(() => null),
    };
  ...
  it("treats a missing active session as already archived for stale sidebar cleanup", async () => {
    const { createSessionsRoute } = await import("../server/routes/sessions.js");
    const app = new Hono();
    const agentsDir = path.join(tmpDir, "agents");
    const sessionPath = path.join(agentsDir, "deleted", "sessions", "missing.jsonl");
    ...
    expect(res.status).toBe(200);
    expect(data).toEqual({ ok: true, missing: true });
  ...
  it("infers subagent agent identity from child sessionPath when history details are missing", async () => {
    ...
    vi.mocked(msgUtils.loadSessionHistoryMessages).mockResolvedValueOnce([
      { role: "assistant", content: "parent says hi" },
      {
        ...
        details: {
          taskId: "subagent-1",
          task: "do work",
          sessionPath: "/tmp/agents/openZetcX/subagent-sessions/child.jsonl",
          streamStatus: "done",
        },
      },
    ]);
    ...
    const engine = {
      ...
      getAgent: vi.fn((id) => (id === "openZetcX" ? { agentName: "openZetcX" } : null)),
    };
    ...
    expect(data.blocks[0]).toMatchObject({
      ...
      agentId: "openZetcX",
      agentName: "openZetcX",
      streamKey: "/tmp/agents/openZetcX/subagent-sessions/child.jsonl",
    });
  });
  ...
  it("prefers explicit executor metadata over owner-path inference", async () => {
    ...
    const engine = {
      ...
      getAgent: vi.fn((id) => {
        if (id === "openZetcX") return { agentName: "openZetcX" };
  ...
  it("uses child-session executor snapshot when live agent has been deleted", async () => {
    ...
    const agentsDir = path.join(tmpDir, "agents");
    const childSessionPath = path.join(agentsDir, "openZetcX", "subagent-sessions", "child.jsonl");
  ...
  it("marks running subagent block done only after deferred store resolves", async () => {
    ...
    expect(res.status).toBe(200);
    expect(msgUtils.loadLatestAssistantSummaryFromSessionFile).toHaveBeenCalledWith("/tmp/agents/openZetcX/subagent-sessions/child.jsonl");
    expect(data.blocks[0]).toMatchObject({
      type: "subagent",
      streamStatus: "done",
