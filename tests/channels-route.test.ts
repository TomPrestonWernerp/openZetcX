...
describe("channels route membership contract", () => {
  ...
  it("freezes channel and phone settings routes when channels are disabled", async () => {
    ...
    const requests = [
      ...
      () => app.request("/api/channels/ch_crew/workspace", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ workspaceRoot: path.join(tmpDir, "workspace") }),
      }),
  ...
  it("persists and returns the channel workspace root", async () => {
    const channelsDir = path.join(tmpDir, "channels");
    await createChannel(channelsDir, {
      id: "ch_crew",
      name: "Crew",
    ...
    const workspaceRoot = path.join(tmpDir, "OH-Works", "Crew");

    const setRes = await app.request("/api/channels/ch_crew/workspace", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    expect(setRes.status).toBe(200);
    expect(await setRes.json()).toMatchObject({ ok: true, workspaceRoot });
    expect(getChannelMeta(path.join(channelsDir, "ch_crew.md")).workspaceRoot).toBe(workspaceRoot);

    const getRes = await app.request("/api/channels/ch_crew");
    expect(await getRes.json()).toMatchObject({ workspaceRoot });

    const listRes = await app.request("/api/channels");
    const listData = await listRes.json();
    expect(listData.channels[0]).toMatchObject({ id: "ch_crew", workspaceRoot });
  });

  it("accepts a workspace root when creating a channel", async () => {
    const channelsDir = path.join(tmpDir, "channels");
    const workspaceRoot = path.join(tmpDir, "OH-Works", "Team");
    ...
    expect(createData).toMatchObject({ ok: true, name: "Team", workspaceRoot });
    expect(getChannelMeta(path.join(channelsDir, `${createData.id}.md`)).workspaceRoot).toBe(workspaceRoot);
  });
});
