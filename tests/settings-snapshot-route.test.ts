...
describe("settings snapshot route", () => {
  ...
  it("renders and persists identity placeholders in the unified settings snapshot", async () => {
    ...
    await writeFile(
      path.join(engine.agentsDir, "agent-a", "identity.md"),
      "# {{agentName}}\n{{userName}} personal assistant",
    );
    await writeFile(
      path.join(engine.agentsDir, "agent-a", "public-ishiki.md"),
      "{{agentName}} public",
    );
    const app = new Hono();
    app.route("/api", createSettingsSnapshotRoute(engine));
    ...
    expect(body.config.agent.yuan).toBe("openZetcX");
