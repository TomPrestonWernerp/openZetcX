...
// openZetcX custom tools — in production these come from agent.tools getter
...
function allNames() {
  return [
    ...
    ...OPENZETCX_CUSTOM_OBJS.map((t) => t.name),
  ];
}
...
describe("session-coordinator tool snapshot (createSession)", () => {
  ...
  beforeEach(() => {
    ...
    coord = new SessionCoordinator({
      ...
      getSkills: () => null,
      buildTools: () => ({ tools: SDK_BUILTIN_OBJS, customTools: OPENZETCX_CUSTOM_OBJS }),
      emitEvent: vi.fn(),
      emitDevLog: vi.fn(),
      getHomeCwd: () => tmpDir,
  ...
  it("Case C: fresh sessions exclude plugin tools disabled by runtime agent config", async () => {
    ...
    coord._d.buildTools = () => ({
      ...
      customTools: [...OPENZETCX_CUSTOM_OBJS, mcpTool],
    });
  ...
  it("Case A: restore keeps newly registered tools inactive when they are absent from the frozen snapshot", async () => {
    ...
    coord._d.buildTools = () => ({
      tools: SDK_BUILTIN_OBJS,
      customTools: [...OPENZETCX_CUSTOM_OBJS, dynamicTool],
    });
