...
smokeDescribe("win32 packaged smoke", () => {
  ...
  it("4. restricted-token sandbox helper launches when the helper is present", async () => {
    ...
    const openZetcXHome = fs.mkdtempSync(path.join(os.tmpdir(), "hana-smoke-home-"));
    try {
      const { deriveSandboxPolicy } = await import("../../lib/sandbox/policy.ts");
      const policy = deriveSandboxPolicy({
        agentDir: openZetcXHome,
        cwd: workDir,
        workspace: workDir,
        workspaceFolders: [],
        openZetcXHome,
      ...
      const exec = (await loadExec())({ sandbox: { policy, openZetcXHome, helperPath: helper } });
      const result = await exec("ipconfig", workDir, {
        onData: () => {},
        signal: undefined,
        timeout: 60,
      ...
      fs.rmSync(openZetcXHome, { recursive: true, force: true });
    }
  });

  it("5. node-pty terminal launches and exits cleanly", async () => {
    const openZetcXHome = fs.mkdtempSync(path.join(os.tmpdir(), "hana-smoke-term-"));
    try {
      const { TerminalSessionManager } = await import("../../lib/terminal/terminal-session-manager.ts");
      const manager = new TerminalSessionManager({ openZetcXHome });
      const sessionPath = path.join(openZetcXHome, "smoke-session.jsonl");
