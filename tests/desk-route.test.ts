...
describe("desk route", () => {
  ...
  it("manages workspace skills through a mounted Studio workspace", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hana-desk-route-"));
    try {
      const openZetcXHome = path.join(tempRoot, "hana");
      ...
      upsertStudioMount(openZetcXHome, {
        schemaVersion: 1,
        mountId: "mount_docs",
  ...
  it("allows explicit desk dirs from workspace scope and rejects arbitrary siblings", async () => {
    ...
    try {
      ...
      const engine = {
        openZetcXHome: path.join(tempRoot, "hana-home"),
        deskCwd: cwd,
        homeCwd: cwd,
  ...
  it("allows a persisted channel workspace to load outside the selected agent sandbox", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hana-desk-route-"));
    try {
      const agentHome = path.join(tempRoot, "agent-home");
      const channelsDir = path.join(tempRoot, "channels");
      const channelWorkspace = path.join(tempRoot, "channel-workspaces", "team");
      fs.mkdirSync(agentHome, { recursive: true });
      fs.mkdirSync(channelsDir, { recursive: true });
      fs.mkdirSync(channelWorkspace, { recursive: true });
      fs.writeFileSync(path.join(channelWorkspace, "visible.txt"), "ok");
      fs.writeFileSync(
        path.join(channelsDir, "ch_team.md"),
        ["---", "id: ch_team", "name: Team", `workspaceRoot: ${channelWorkspace}`, "---", ""].join("\n"),
        "utf-8",
      );
      ...
      const res = await app.request(`/api/desk/files?dir=${encodeURIComponent(channelWorkspace)}&agentId=open`);

      expect(res.status).toBe(200);
      const data = await res.json();
      expect(data.basePath).toBe(channelWorkspace);
      expect(data.files.map(f => f.name)).toContain("visible.txt");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it("allows local file uploads into a persisted channel workspace outside the selected agent sandbox", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hana-desk-route-"));
    try {
      const agentHome = path.join(tempRoot, "agent-home");
      const channelsDir = path.join(tempRoot, "channels");
      const channelWorkspace = path.join(tempRoot, "channel-workspaces", "team");
      const uploadSourceDir = path.join(tempRoot, "drag-source");
      const uploadSource = path.join(uploadSourceDir, "brief.txt");
      fs.mkdirSync(agentHome, { recursive: true });
      fs.mkdirSync(channelsDir, { recursive: true });
      fs.mkdirSync(channelWorkspace, { recursive: true });
      fs.mkdirSync(uploadSourceDir, { recursive: true });
      fs.writeFileSync(uploadSource, "shared draft", "utf-8");
      fs.writeFileSync(
        path.join(channelsDir, "ch_team.md"),
        ["---", "id: ch_team", "name: Team", `workspaceRoot: ${channelWorkspace}`, "---", ""].join("\n"),
        "utf-8",
      ...
      expect(res.status).toBe(200);
      const data = await res.json();
      expect(data.results).toEqual([{ src: uploadSource, name: "brief.txt" }]);
      expect(fs.readFileSync(path.join(channelWorkspace, "brief.txt"), "utf-8")).toBe("shared draft");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });

  it("allows channel workspaces saved with snake case settings", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hana-desk-route-"));
    try {
      const agentHome = path.join(tempRoot, "agent-home");
      const channelWorkspace = path.join(tempRoot, "channel-workspaces", "team");
      ...
      const engine = {
        ...
        config: {
          channel: {
            workspaces: {
              ch_team: { workspace_root: channelWorkspace },
            },
          },
        },
        deskCwd: agentHome,
  ...
  it("resolves relative channel workspace names under OH-Works for listing and uploads", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hana-desk-route-"));
    try {
      const workspaceRoot = path.join(tempRoot, "selected-workspace");
      const agentHome = path.join(tempRoot, "agent-home");
      const channelsDir = path.join(tempRoot, "channels");
      const channelName = "测试集群";
      const channelWorkspace = path.join(workspaceRoot, WORKSPACE_OUTPUT_ROOT_DIRNAME, channelName);
      fs.mkdirSync(agentHome, { recursive: true });
      fs.mkdirSync(channelsDir, { recursive: true });
      fs.mkdirSync(channelWorkspace, { recursive: true });
      fs.writeFileSync(path.join(channelWorkspace, "visible.txt"), "ok", "utf-8");
      fs.writeFileSync(
        path.join(channelsDir, "ch_team.md"),
        ["---", "id: ch_team", "name: Team", `workspaceRoot: ${channelName}`, "---", ""].join("\n"),
        "utf-8",
      );

      const engine = {
        openZetcXHome: path.join(tempRoot, "hana-home"),
        config: { cwd_history: [workspaceRoot] },
        channelsDir,
        deskCwd: agentHome,
        homeCwd: agentHome,
        isApprovedDeskDir: vi.fn(() => false),
      ...
      const absoluteListed = await app.request(`/api/desk/files?dir=${encodeURIComponent(channelWorkspace)}&agentId=open`);
      expect(absoluteListed.status).toBe(200);
      const absoluteData = await absoluteListed.json();
      expect(absoluteData.basePath).toBe(channelWorkspace);
      expect(absoluteData.files.map(f => f.name)).toContain("visible.txt");

      const listed = await app.request(`/api/desk/files?dir=${encodeURIComponent(channelName)}&agentId=open`);
      expect(listed.status).toBe(200);
      const listData = await listed.json();
      expect(listData.basePath).toBe(channelWorkspace);
      expect(listData.files.map(f => f.name)).toContain("visible.txt");
      ...
      const uploaded = await app.request("/api/desk/files", {
        ...
        body: JSON.stringify({
          ...
          files: [{
            name: "uploaded.txt",
            contentBase64: Buffer.from("shared upload").toString("base64"),
          }],
        }),
      });
      ...
      expect(uploadData.results).toMatchObject([{ name: "uploaded.txt", ok: true }]);
      expect(fs.readFileSync(path.join(channelWorkspace, "uploaded.txt"), "utf-8")).toBe("shared upload");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("moves workspace tree items by explicit subdir and reports affected folders", async () => {
    ...
    try {
      ...
      const engine = {
        openZetcXHome: path.join(tempRoot, "hana"),
        deskCwd: cwd,
        homeCwd: cwd,
      };
  ...
  it("treats mkdir of an existing directory as success", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hana-desk-route-"));
    try {
      const cwd = path.join(tempRoot, "workspace");
      ...
      const res = await app.request("/api/desk/files", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "mkdir", dir: cwd, name: "Crew" }),
      });

      expect(res.status).toBe(200);
      expect(await res.json()).toMatchObject({ ok: true });
      expect(fs.existsSync(path.join(cwd, "Crew"))).toBe(true);
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("allows uploaded File content into the active desk directory for non-local-owner principals", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hana-desk-route-"));
    try {
      const cwd = path.join(tempRoot, "workspace");
      fs.mkdirSync(cwd, { recursive: true });
      ...
      const res = await app.request("/api/desk/files", {
        ...
        body: JSON.stringify({
          ...
          files: [{
            name: "brief.md",
            type: "text/markdown",
            contentBase64: Buffer.from("shared brief", "utf-8").toString("base64"),
          }],
        }),
      });
      ...
      expect(res.status).toBe(200);
      const data = await res.json();
      expect(data.results).toEqual([{ name: "brief.md", ok: true, size: 12 }]);
      expect(fs.readFileSync(path.join(cwd, "notes", "brief.md"), "utf-8")).toBe("shared brief");
      expect(data.files.map(f => f.name)).toContain("brief.md");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
