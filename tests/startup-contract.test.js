...
describe("local startup contract", () => {
  ...
  it("desktop only reuses a server that advertises the current API contract", () => {
    const mainCjs = fs.readFileSync(path.join(ROOT, "desktop", "main.cjs"), "utf-8");
    const serverSource = fs.readFileSync(path.join(ROOT, "server", "index.js"), "utf-8");

    expect(serverSource).toContain("SERVER_API_CONTRACT_VERSION");
    expect(serverSource).toContain("sessionsSwitch: true");
    expect(serverSource).toContain("pluginMarketplace: true");
    expect(mainCjs).toContain("REQUIRED_SERVER_API_CONTRACT_VERSION");
    expect(mainCjs).toContain("health?.apiContractVersion === REQUIRED_SERVER_API_CONTRACT_VERSION");
    expect(mainCjs).toContain("health?.capabilities?.sessionsSwitch === true");
    expect(mainCjs).toContain("health?.capabilities?.pluginMarketplace === true");
  });
