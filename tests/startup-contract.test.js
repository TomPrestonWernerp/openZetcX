...
describe("local startup contract", () => {
  ...
  it("CLI and server configure the Pi SDK agent directory from HANA_HOME", () => {
    ...
    expect(cliSource).toContain("ensureHanaPiSdkDirs(openZetcXHome)");
    expect(cliSource).toContain("configureProcessPiSdkEnv(openZetcXHome)");
    expect(serverSource).toContain("ensureHanaPiSdkDirs(openZetcXHome)");
    expect(serverSource).toContain("configureProcessPiSdkEnv(openZetcXHome)");
  });

  it("desktop main propagates Hana-owned Pi SDK env to the spawned server", () => {
    const mainCjs = fs.readFileSync(path.join(ROOT, "desktop", "main.cjs"), "utf-8");

    expect(mainCjs).toContain("ensureHanaPiSdkDirs(openZetcXHome)");
    expect(mainCjs).toContain("configureProcessPiSdkEnv(openZetcXHome)");
    expect(mainCjs).toContain("withHanaPiSdkEnv(process.env, openZetcXHome)");
  });
