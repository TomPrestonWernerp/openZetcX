...
describe("HanaEngine resource loader options", () => {
  ...
  it("uses explicit Hana-owned Pi SDK cwd, agentDir, and in-memory Pi settings", () => {
    ...
    engine.openZetcXHome = "/openZetcX-home";
    engine._agentMgr = {
      activeAgentId: "agent-a",
      agent: {
        agentDir: "/openZetcX-home/agents/agent-a",
        systemPrompt: "agent prompt",
      },
    };
    engine.getHomeCwd = vi.fn(() => "/workspace-a");

    const options = engine._createResourceLoaderOptions("/openZetcX-home/skills");

    expect(options).toMatchObject({
      cwd: path.join("/openZetcX-home", ".pi", "project"),
      agentDir: path.join("/openZetcX-home", ".pi", "agent"),
      ...
      noThemes: true,
      additionalSkillPaths: ["/openZetcX-home/skills"],
    });
