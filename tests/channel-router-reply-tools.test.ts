...
describe("ChannelRouter reply tool boundary", () => {
  ...
  it("runs channel phone delivery from the channel workspace when configured", async () => {
    runAgentPhoneSessionMock.mockClear();

    const root = fs.mkdtempSync(path.join(os.tmpdir(), "hana-channel-workspace-"));
    const channelsDir = path.join(root, "channels");
    const workspaceRoot = path.join(root, "OH-Works", "crew");
    fs.mkdirSync(channelsDir, { recursive: true });
    fs.writeFileSync(
      path.join(channelsDir, "ch_crew.md"),
      [
    ...
    await router._executeReply(
      "hanako",
      "ch_crew",
      "user: @Hanako please create a file",
    );
