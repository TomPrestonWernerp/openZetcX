...
export interface openZetcXCreateInput {
  id?: string;
  name: string;
  yuan?: string;
  ownerPluginId?: string | null;
...
export interface openZetcXUpdateInput {
  name?: string;
  yuan?: string;
  ownerPluginId?: string | null;
  visibility?: 'public' | 'plugin_private' | 'private' | string;
...
export function getPluginRequestContext(c: HanaPluginHonoLikeContext): HanaPluginRouteRequestContext {
  ...
  if (!requestContext || typeof requestContext !== 'object') {
    throw new Error('getPluginRequestContext must be called inside a openZetcX plugin route handler');
  }
