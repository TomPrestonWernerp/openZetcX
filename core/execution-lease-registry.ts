...
export function ensureExecutionLeaseRegistry(openZetcXHome, { now = new Date().toISOString() } = {}) {
  const filePath = executionLeaseRegistryPath(openZetcXHome);
...
export function loadExecutionLeaseRegistry(openZetcXHome, { now = new Date().toISOString() } = {}) {
  ensureExecutionLeaseRegistry(openZetcXHome, { now });
  return validateExecutionLeaseRegistry(
    readJsonRequired(executionLeaseRegistryPath(openZetcXHome), EXECUTION_LEASES_FILE),
    EXECUTION_LEASES_FILE,
    { now },
  );
}

export function issueExecutionLease(openZetcXHome, lease, { now = new Date().toISOString() } = {}) {
  const registry = loadExecutionLeaseRegistry(openZetcXHome, { now });
  ...
  persistExecutionLeaseRegistry(openZetcXHome, registry);
  return clonePlain(record);
}

export function consumeExecutionLease(openZetcXHome, leaseId, { now = new Date().toISOString() } = {}) {
  const registry = loadExecutionLeaseRegistry(openZetcXHome, { now });
...
export function revokeExecutionLease(openZetcXHome, leaseId, { now = new Date().toISOString() } = {}) {
  const registry = loadExecutionLeaseRegistry(openZetcXHome, { now });
  const lease = registry.leases.find((item) => item.leaseId === leaseId);
  if (!lease) throw new Error(`execution lease not found: ${leaseId}`);
...
export function persistExecutionLeaseRegistry(openZetcXHome, registry) {
  validateExecutionLeaseRegistry(registry, EXECUTION_LEASES_FILE, { validateExpiry: false });
  writeJsonAtomic(executionLeaseRegistryPath(openZetcXHome), registry);
}

export function executionLeaseRegistryPath(openZetcXHome) {
  if (!isNonEmptyString(openZetcXHome)) throw new Error("openZetcXHome required");
  return path.join(openZetcXHome, SECURITY_DIR, EXECUTION_LEASES_FILE);
}
