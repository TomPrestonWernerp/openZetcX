...
export function issuePluginIframeTicket({
...
}: { openZetcXHome?: string; pluginId?: string; surfacePath?: string; principalId?: string; now?: string; ttlMs?: number } = {}) {
  assertNonEmpty(openZetcXHome, "openZetcXHome");
  ...
  const signature = signBody(openZetcXHome, body);
  return {
    ...payload,
...
export function verifyPluginIframeTicket({
  ...
  surfacePath,
  now = new Date().toISOString(),
}: { openZetcXHome?: string; ticket?: string; pluginId?: string; surfacePath?: string; now?: string } = {}) {
  assertNonEmpty(openZetcXHome, "openZetcXHome");
  assertNonEmpty(pluginId, "pluginId");
  assertNonEmpty(surfacePath, "surfacePath");
  ...
  const expected = signBody(openZetcXHome, body);
...
function pluginIframeTicketKeyPath(openZetcXHome) {
  assertNonEmpty(openZetcXHome, "openZetcXHome");
  return path.join(openZetcXHome, "security", PLUGIN_IFRAME_TICKET_KEY_FILE);
}

function signBody(openZetcXHome, body) {
  return crypto
    .createHmac("sha256", readOrCreateTicketKey(openZetcXHome))
    .update(body)
    .digest("base64url");
}

function readOrCreateTicketKey(openZetcXHome) {
  const filePath = pluginIframeTicketKeyPath(openZetcXHome);
  try {
