...
export class StudioCronService {
   ...
   */
  constructor({ openZetcXHome, agentsDir, getStudioId }) {
    if (!openZetcXHome) throw new Error("StudioCronService requires openZetcXHome");
  ...
  _getStore() {
    ...
    if (!this._store || this._storeStudioId !== studioId) {
      const deskDir = path.join(this._openZetcXHome, "studios", studioId, "desk");
  ...
  _importLegacyJobs(store, studioId) {
    const studioRunsDir = path.join(this._openZetcXHome, "studios", studioId, "desk", "cron-runs");
