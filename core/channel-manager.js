 ...
 * ChannelManager — 集群管理
 *
 * 从 Engine 提取，负责集群 CRUD、成员管理、新 agent 集群初始化。
...
export class ChannelManager {
  /**
   * @param {object} deps
   * @param {string} deps.channelsDir - 集群目录
    ...
    for (const f of channelFiles) {
      ...
      try {
        ...
        if (remaining.length <= 1) {
          ...
          log.log(`集群 "${channelId}" 成员不足，已删除`);
        }
      } catch (err) {
        log.error(`清理集群 "${channelId}" 失败: ${err.message}`);
      }
    }
  ...
  async deleteChannelByName(channelId) {
    ...
    log.log(`已删除集群: ${channelId}`);
  }

  /**
   * 触发集群立即 triage（用户发消息后调用）
   */
  ...
  async setupChannelsForNewAgent(agentId) {
    ...
    // 写 agent 的 channels.md（扫描所有集群，加入包含该 agent 的）
   ...
   * 修复 agent 的集群游标投影。
   *
   * 集群文件 frontmatter 的 members 是成员真相源；agent 的 channels.md 只保存
   * last-read cursor。老数据或异常中断导致 cursor 缺失时，在启动阶段补齐为
   * never，不改集群文件，也不删除旧 cursor，避免改变用户可见历史。
   */
  async repairChannelCursorProjection() {
    ...
    if (added > 0) {
      log.log(`已修复 ${added} 个集群游标投影`);
    }
    return { added };
  }

  /** 清理被删集群的 bookmark（从其他 agent 和用户的 bookmark 中移除） */
