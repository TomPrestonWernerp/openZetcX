...
export class Agent {
  ...
  async init(log = () => {}, sharedModels = {}, resolveModel = null) {
    ...
    // 9. 集群工具 + 私信工具（需要 channelsDir 和 agentsDir）
   ...
   * 用于"非 session"路径（巡检/cron/集群/DM/bridge owner 新建快照），
  ...
  buildSystemPrompt(options = {}) {
    ...
    const parts = [
      isZh
        ? "你运行在 openZetcX 平台上，由 liliMozi 开发。项目主页：https://github.com/liliMozi/openZetcX"
        : "You are running on the openZetcX platform, developed by liliMozi. Project page: https://github.com/liliMozi/openZetcX",
    ];
