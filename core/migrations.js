 ...
 * 又让从没用过集群的大多数用户默认关闭（产品判断：bug 修之前 ticker 无条件跑，
  ...
  for (const sessionPath of sessionPaths) {
    ...
    for (const line of lines) {
      ...
      for (const ref of legacySessionFileRefs(msg)) {
        const ok = registerLegacySessionFile({ registry, sessionPath, ref, openZetcXHome, log });
        if (ok) registered++;
        else skipped++;
      }
...
function registerLegacySessionFile({ registry, sessionPath, ref, openZetcXHome, log }) {
