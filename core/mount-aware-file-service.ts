...
export class MountAwareFileService {
  ...
  _resolveRootInternal(rootId = "default") {
    ...
    const mount = findLocalFsMount(this._openZetcXHome, this._studioId, id);
...
function createServiceResourceIO({ openZetcXHome, defaultRoot, studioId }) {
...
function findLocalFsMount(openZetcXHome, studioId, rootId) {
  if (!studioId) return null;
  let registry;
  try {
    registry = loadStudioMountRegistry(openZetcXHome);
  } catch {
    return null;
  }
