 ...
 */
export function migrateLegacyApiKeyAuthToProviders({ openZetcXHome, providerRegistry, log = () => {} }: { openZetcXHome: string; providerRegistry: any; log?: (msg: string) => void }) {
  if (!openZetcXHome) return { migrated: 0, providers: [] };

  const authPath = path.join(openZetcXHome, "auth.json");
  ...
  const modelsJsonProvidersRaw = readJson(path.join(openZetcXHome, "models.json")).providers || {};
