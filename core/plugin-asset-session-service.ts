...
function pluginAssetSessionKeyPath(openZetcXHome) {
  assertNonEmpty(openZetcXHome, "openZetcXHome");
  return path.join(openZetcXHome, "security", PLUGIN_ASSET_SESSION_KEY_FILE);
}
...
function readOrCreateSessionKey(openZetcXHome) {
  const filePath = pluginAssetSessionKeyPath(openZetcXHome);
  try {
