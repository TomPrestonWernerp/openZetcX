...
export class ProviderRegistry {
  /**
   * @param {string} openZetcXHome - 用户数据根目录（如 ~/.openZetcX-dev）
   */
  ...
  migrateOverridesToAddedModels(agentsDir, log = () => {}) {
    ...
    for (const dir of agentDirs) {
      ...
      if (cfgChanged) {
        ...
        const header = "# openZetcX Agent 配置\n# 由设置页面管理，手动编辑也可以\n\n";
  ...
  /** 从 _openZetcXHome 直接读 added-models.yaml（mtime 缓存，文件未变时跳过磁盘读取） */
  _loadAddedModels() {
    const ymlPath = path.join(this._openZetcXHome, "added-models.yaml");
    try {
  ...
  /** 将 providers 对象写入 _openZetcXHome/added-models.yaml */
  ...
  _readOAuthEntry(authJsonKey) {
    try {
      const authPath = path.join(this._openZetcXHome, "auth.json");
