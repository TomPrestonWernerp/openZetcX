...
export class HanaEngine {
   ...
   */
  constructor({ openZetcXHome, productDir, agentId, appVersion }) {
    ...
    this._usageLedger = createUsageLedger({
      storagePath: path.join(this.openZetcXHome, "usage-ledger.json"),
      eventBus: {
  ...
  _openSessionManifestStore() {
    const dbPath = path.join(this.openZetcXHome, "session-manifest.db");
    try {
  ...
  async init(log: any = () => {}) {
    ...
    this._resourceAccess = new ResourceAccessService({
      ...
      audit: (event) => appendSecurityAuditEvent(this.openZetcXHome, event),
    });
