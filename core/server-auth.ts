...
export function createServerAuthService({
  ...
  function authenticateRequestDetailed({
    ...
    if (!parsed) {
      const webPrincipal = authenticateWebSession(openZetcXHome, cookieHeader, { now });
      if (!webPrincipal) {
    ...
    const devicePrincipal = authenticateDeviceCredential(openZetcXHome, parsed.token, { now });
    if (!devicePrincipal) {
