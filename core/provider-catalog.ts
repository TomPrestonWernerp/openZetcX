...
export class ProviderCatalogStore {
  ...
  constructor(openZetcXHome: string) {
    if (!openZetcXHome) throw new Error("ProviderCatalogStore requires openZetcXHome");
    this._openZetcXHome = openZetcXHome;
  }

  get catalogPath() {
    return path.join(this._openZetcXHome, PROVIDER_CATALOG_FILE);
  }

  get legacyAddedModelsPath() {
    return path.join(this._openZetcXHome, LEGACY_ADDED_MODELS_FILE);
  }
