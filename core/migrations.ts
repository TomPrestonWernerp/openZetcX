...
const migrations = {
  ...
  // 设计增强默认显式关闭；旧 Agent 配置只有用户手动开启后才可用
...
/**
 * @param {object} ctx
 * @param {string}   ctx.openZetcXHome
...
function migrateSubagentExecutorMetadata(ctx) {
  const { agentsDir, openZetcXHome, log } = ctx;
  ...
  const deferredTasksPath = path.join(openZetcXHome, ".ephemeral", "deferred-tasks.json");
  try {
...
function migrateMiniMaxTokenPlanAnthropicEndpoint(ctx) {
  const { openZetcXHome, log } = ctx;
  const ymlPath = path.join(openZetcXHome, "added-models.yaml");
 ...
 *
 * 覆盖位置：
 *   1. ~/.openZetcX/added-models.yaml 的 providers.*.models[] 数组（用户主战场）
 *   2. ~/.openZetcX/agents/*\/config.yaml 的 models.overrides（历史残留兜底）
 *
 * 幂等：只在发现 vision 字段时改写；image 已存在时保留不覆盖。
 * 配合读时兼容（model-sync.js、provider-registry.js）形成双保险。
 */
function migrateVisionToImage(ctx) {
  const { openZetcXHome, agentsDir, log } = ctx;
  let ymlCount = 0;
  let overrideCount = 0;
...
function migrateCronJobsToAutomationReadModel(ctx) {
  ...
  const studiosDir = path.join(openZetcXHome, "studios");
  try {
...
function migrateProviderCatalogV2Cutover(ctx) {
  const { openZetcXHome, providerRegistry, log } = ctx;
  const store = providerRegistry?._catalog || new ProviderCatalogStore(openZetcXHome);
...
function migrateCodexImageGenerationDefaultsToResolutionSchema(ctx) {
  const { openZetcXHome, prefs, log } = ctx;
  const preferences = prefs.getPreferences();
  ...
  const pluginChanged = removeCodexImageSizeDefaultFromPluginConfig(openZetcXHome, log);
...
function removeCodexImageSizeDefaultFromPluginConfig(openZetcXHome, log) {
  const configPath = path.join(openZetcXHome, "plugin-data", "image-gen", "config.json");
...
function migrateLearnedSkillsToGlobalSkillPool(ctx) {
  const { openZetcXHome, agentsDir, log } = ctx;
  const skillsDir = path.join(openZetcXHome, "skills");
...
function backfillLegacySessionFiles(ctx) {
  const { openZetcXHome, agentsDir, log } = ctx;
  if (!openZetcXHome || !agentsDir) return;
  ...
  for (const sessionPath of sessionPaths) {
    ...
    for (const line of lines) {
      ...
      for (const ref of legacySessionFileRefs(msg)) {
        const ok = registerLegacySessionFile({ registry, sessionId, sessionPath, ref, openZetcXHome, log });
        if (ok) registered++;
        else skipped++;
      }
...
function repairModelsJsonPiInputSchema(ctx) {
  const modelsJsonPath = path.join(ctx.openZetcXHome, "models.json");
  let raw;
  try {
...
function promoteAgentVideoOverrides(ctx) {
  const { openZetcXHome, agentsDir } = ctx;
  const ymlPath = path.join(openZetcXHome, "added-models.yaml");
  const raw = safeReadYAMLSync(ymlPath, null, YAML);
  if (!raw?.providers || typeof raw.providers !== "object") return 0;
...
function registerLegacySessionFile({ registry, sessionId = null, sessionPath, ref, openZetcXHome, log }) {
  ...
  try {
    registry.registerFile({
      ...
      storageKind: normalizeLegacyStorageKind(ref, openZetcXHome),
    });
    return true;
  } catch (err) {
...
function normalizeLegacyStorageKind(ref, openZetcXHome) {
  const storageKind = ref.storageKind || "external";
  if (storageKind !== "managed_cache") return storageKind;

  const managedRoot = path.join(openZetcXHome, "session-files");
...
function migrateLocalIdentityRegistries(ctx) {
  const { openZetcXHome, log } = ctx;
  const { created, migratedFromLegacySpaces } = ensureLocalIdentityRegistries(openZetcXHome);
...
function migrateRemoteAccessFoundationRegistries(ctx) {
  const { openZetcXHome, log } = ctx;
  const { created } = ensureRemoteAccessFoundationRegistries(openZetcXHome);
...
function migrateDurableSubagentRunRegistry(ctx) {
  const { openZetcXHome, agentsDir, log } = ctx;
  const store = new SubagentRunStore(path.join(openZetcXHome, "subagent-runs.json"));
  let imported = 0;
...
function migrateSubagentThreadRegistry(ctx) {
  const { openZetcXHome, log } = ctx;
  const threadStore = new SubagentThreadStore(path.join(openZetcXHome, "subagent-threads.json"));
  const runStore = new SubagentRunStore(path.join(openZetcXHome, "subagent-runs.json"));
  let importedRuns = 0;
  ...
  const reusableRaw = readJsonForMigration(path.join(openZetcXHome, "reusable-subagents.json"));
...
function migrateSubagentDirectThreadSemantics(ctx) {
  const { openZetcXHome, log } = ctx;
  const threadsPath = path.join(openZetcXHome, "subagent-threads.json");
  const runsPath = path.join(openZetcXHome, "subagent-runs.json");
  let threadCount = 0;
  let runCount = 0;
...
function migrateChannelPhoneSettingsDefaults(ctx) {
  const { openZetcXHome, log } = ctx;
  const channelsDir = path.join(openZetcXHome, "channels");
