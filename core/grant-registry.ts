...
export function ensureGrantRegistry(openZetcXHome, { now = new Date().toISOString() } = {}) {
  const filePath = grantRegistryPath(openZetcXHome);
...
export function loadGrantRegistry(openZetcXHome) {
  ensureGrantRegistry(openZetcXHome);
  return validateGrantRegistry(readJsonRequired(grantRegistryPath(openZetcXHome), GRANTS_FILE), GRANTS_FILE);
}

export function createGrant(openZetcXHome, input: Record<string, any> = {}) {
  const now = input.now || new Date().toISOString();
  const registry = loadGrantRegistry(openZetcXHome);
  ...
  persistGrantRegistry(openZetcXHome, registry);
  return clonePlain(grant);
}

export function findActiveGrantsForPrincipal(openZetcXHome, principalId, { now = new Date().toISOString() } = {}) {
  ...
  if (changed) persistGrantRegistry(openZetcXHome, registry);
  return active;
}

export function revokeGrant(openZetcXHome, grantId, { now = new Date().toISOString() } = {}) {
...
export function persistGrantRegistry(openZetcXHome, registry) {
  validateGrantRegistry(registry, GRANTS_FILE);
  writeJsonAtomic(grantRegistryPath(openZetcXHome), registry);
}

export function grantRegistryPath(openZetcXHome) {
  if (!isNonEmptyString(openZetcXHome)) throw new Error("openZetcXHome required");
  return path.join(openZetcXHome, SECURITY_DIR, GRANTS_FILE);
}
