...
export function loadServerIdentity(openZetcXHome) {
  const serverNode = readRequiredIdentityJson(path.join(openZetcXHome, SERVER_NODE_FILE), SERVER_NODE_FILE);
  const users = readRequiredIdentityJson(path.join(openZetcXHome, USERS_FILE), USERS_FILE);
  const studios = readRequiredStudioRegistry(openZetcXHome);
...
export function ensureLocalIdentityRegistries(openZetcXHome) {
  const serverNodePath = path.join(openZetcXHome, SERVER_NODE_FILE);
  const usersPath = path.join(openZetcXHome, USERS_FILE);
  const studiosPath = path.join(openZetcXHome, STUDIOS_FILE);
  const legacySpacesPath = path.join(openZetcXHome, LEGACY_SPACES_FILE);
  ...
  const foundationRegistries = ensureRemoteAccessFoundationRegistries(openZetcXHome, { now });
...
export function ensureRemoteAccessFoundationRegistries(openZetcXHome, { now = new Date().toISOString() } = {}) {
  return {
    created: [
      ...ensureDeviceAccessRegistries(openZetcXHome, { now }).created,
      ...ensureServerNetworkConfig(openZetcXHome, { now }).created,
      ...ensureStudioMountRegistry(openZetcXHome, { now }).created,
      ...ensureSecurityRegistries(openZetcXHome, { now }).created,
    ],
  };
}

function ensureSecurityRegistries(openZetcXHome, { now }) {
  const created = [];
  const grantPath = path.join(openZetcXHome, "security", "grants.json");
  const leasePath = path.join(openZetcXHome, "security", "execution-leases.json");
  const hadGrant = fs.existsSync(grantPath);
  const hadLease = fs.existsSync(leasePath);
  ensureGrantRegistry(openZetcXHome, { now });
  ensureExecutionLeaseRegistry(openZetcXHome, { now });
  if (!hadGrant) created.push(path.join("security", "grants.json"));
  if (!hadLease) created.push(path.join("security", "execution-leases.json"));
  return { created };
}

function readRequiredStudioRegistry(openZetcXHome) {
  const studiosPath = path.join(openZetcXHome, STUDIOS_FILE);
