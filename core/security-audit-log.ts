...
export function securityAuditLogPath(openZetcXHome) {
  if (!openZetcXHome) throw new Error("openZetcXHome required");
  return path.join(openZetcXHome, "logs", SECURITY_AUDIT_LOG_FILE);
}

export function appendSecurityAuditEvent(openZetcXHome, event, {
  now = new Date().toISOString(),
  eventId = `sec_${crypto.randomUUID()}`,
} = {}) {
  if (!openZetcXHome) return null;
  const record = normalizeAuditEvent(event, { now, eventId });
  const filePath = securityAuditLogPath(openZetcXHome);
  try {
