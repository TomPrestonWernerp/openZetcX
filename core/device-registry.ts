...
export function ensureDeviceAccessRegistries(openZetcXHome, { now = new Date().toISOString() } = {}) {
  const created = [];
  ...
  for (const [file, emptyRegistry, validate] of registries) {
    const filePath = path.join(openZetcXHome, file);
...
export function loadDeviceAccessRegistries(openZetcXHome) {
  ensureDeviceAccessRegistries(openZetcXHome);
  const devices = readJsonRequired(path.join(openZetcXHome, DEVICES_FILE), DEVICES_FILE);
  const credentials = readJsonRequired(path.join(openZetcXHome, DEVICE_CREDENTIALS_FILE), DEVICE_CREDENTIALS_FILE);
  const pairingSessions = readJsonRequired(path.join(openZetcXHome, PAIRING_SESSIONS_FILE), PAIRING_SESSIONS_FILE);
...
export function createDeviceCredential(openZetcXHome, input) {
  const now = input?.now || new Date().toISOString();
  const registries = loadDeviceAccessRegistries(openZetcXHome);
  const issued = issueDeviceCredential(registries, input, { now });
  persistDeviceAccessRegistries(openZetcXHome, registries);
  return cloneIssuedCredential(issued);
}

export function authenticateDeviceCredential(openZetcXHome, secret, { now = new Date().toISOString() } = {}) {
...
export function revokeDeviceCredential(openZetcXHome, credentialId, { now = new Date().toISOString() } = {}) {
...
export function revokeDevice(openZetcXHome, deviceId, { now = new Date().toISOString() } = {}) {
...
export function createPairingSession(openZetcXHome, input) {
  const now = input?.now || new Date().toISOString();
...
export function approvePairingSession(openZetcXHome, input) {
  const now = input?.now || new Date().toISOString();
...
function persistDeviceAccessRegistries(openZetcXHome, registries) {
  ...
  writeJsonAtomic(path.join(openZetcXHome, DEVICES_FILE), registries.devices);
  writeJsonAtomic(path.join(openZetcXHome, DEVICE_CREDENTIALS_FILE), registries.credentials);
  writeJsonAtomic(path.join(openZetcXHome, PAIRING_SESSIONS_FILE), registries.pairingSessions);
}
