...
export function issueRemoteWriteLease({
  ...
  if (!openZetcXHome || isLocalOwner(requestContext)) return null;
  ...
  const lease = issueExecutionLease(openZetcXHome, {
    schemaVersion: 1,
...
export function consumeRemoteWriteLease(openZetcXHome, lease, { now = new Date().toISOString() } = {}) {
  if (!openZetcXHome || !lease?.leaseId) return null;
  return consumeExecutionLease(openZetcXHome, lease.leaseId, { now });
}

export function revokeRemoteWriteLease(openZetcXHome, lease, { now = new Date().toISOString() } = {}) {
  if (!openZetcXHome || !lease?.leaseId) return null;
  return revokeExecutionLease(openZetcXHome, lease.leaseId, { now });
}
