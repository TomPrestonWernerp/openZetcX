...
import { assertKnownYuan, getAgentConfigRepairState, normalizeYuanKey } from "./yuan-registry.ts";
...
export function hasAgentTemplatePlaceholders(content) {
  return AGENT_TEMPLATE_PLACEHOLDER_RE.test(String(content || ""));
}

export function renderAgentTemplatePlaceholders(identityMd, cfg, agentId) {
...
function normalizeAgentYuanForMetadata(value) {
  try {
    return normalizeYuanKey(value || "openZetcX");
  } catch {
    return "openZetcX";
  }
}

function resolveOpenZetcXRolePreset(value) {
  return getOpenZetcXRolePreset(value);
}
...
export class AgentManager {
  ...
  _readDeletedAgentInfo(agentId) {
    ...
    return {
      ...
      name,
      agentName: name,
      yuan: normalizeAgentYuanForMetadata(tombstone.yuan || cfg.agent?.yuan),
  ...
  _scanAgentList() {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        let identity = "";
        try {
          const identityPath = path.join(this._d.agentsDir, entry.name, "identity.md");
          const idMd = fs.readFileSync(identityPath, "utf-8");
          const renderedIdMd = renderAgentTemplatePlaceholders(idMd, cfg, entry.name);
          if (renderedIdMd !== idMd && hasAgentTemplatePlaceholders(idMd)) {
            fs.writeFileSync(identityPath, renderedIdMd, "utf-8");
          }
        ...
        agents.push({
          ...
          yuan: normalizeAgentYuanForMetadata(cfg.agent?.yuan),
  ...
  async createAgent({ name, id, yuan, rolePreset, enabledSkills, initialFiles, avatarPath, initialMemory }) {
    ...
    const yuanType = assertKnownYuan(this._d.productDir, yuan || "openZetcX");
    const selectedRolePreset = resolveOpenZetcXRolePreset(rolePreset);
    ...
    if (selectedRolePreset?.identity) {
      fs.writeFileSync(
        path.join(agentDir, "identity.md"),
        renderAgentTemplatePlaceholders(selectedRolePreset.identity, config, agentId),
        "utf-8",
      );
    } else if (identitySrc) {
      const tmpl = fs.readFileSync(identitySrc, "utf-8");
      fs.writeFileSync(path.join(agentDir, "identity.md"), renderAgentTemplatePlaceholders(tmpl, config, agentId), "utf-8");
    }
    ...
    if (selectedRolePreset?.ishiki) {
      fs.writeFileSync(
        path.join(agentDir, "ishiki.md"),
        renderAgentTemplatePlaceholders(selectedRolePreset.ishiki, config, agentId),
        "utf-8",
      );
    } else if (ishikiSrc) {
    ...
    if (publicIshikiSrc) {
      const tmpl = fs.readFileSync(publicIshikiSrc, "utf-8");
      fs.writeFileSync(path.join(agentDir, "public-ishiki.md"), renderAgentTemplatePlaceholders(tmpl, config, agentId), "utf-8");
    }
  ...
  async deleteAgent(agentId) {
    ...
    const tombstone = {
      ...
      yuan: normalizeAgentYuanForMetadata(ag?.config?.agent?.yuan || this._readAgentYuanFromConfig(agentDir)),
    ...
    if (this._d.openZetcXHome) {
      try {
        detachAgentFromBundles({ openZetcXHome: this._d.openZetcXHome }, agentId);
      } catch (err) {
  ...
  _readAgentYuanFromConfig(agentDir) {
    try {
      const cfg = safeReadYAMLSync(path.join(agentDir, "config.yaml"), {}, YAML);
      return normalizeAgentYuanForMetadata(cfg.agent?.yuan);
    } catch {
      return null;
    }
  }
