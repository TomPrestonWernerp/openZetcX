...
function registerDisplayAttachments({ openZetcXHome, sessionPath, attachments, registerSessionFile }) {
  ...
  for (const attachment of attachments || []) {
    ...
    if (!next.fileId && next.path && path.isAbsolute(next.path) && typeof registerSessionFile === "function") {
      sessionFile = serializeSessionFile(registerSessionFile({
        ...
        storageKind: displayAttachmentStorageKind(openZetcXHome, next.path),
      }));
      if (sessionFile) {
        next = {
          ...next,
...
function displayAttachmentStorageKind(openZetcXHome, filePath) {
  if (!openZetcXHome) return "external";
  const root = path.resolve(openZetcXHome, "session-files");
