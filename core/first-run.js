 ...
 */
export function ensureFirstRun(openZetcXHome, productDir) {
  // 1. 确保目录结构存在
  ...
  touchIfMissing(path.join(openZetcXHome, 'user', 'user.md'));
...
function seedDefaultAgent(agentsDir, productDir) {
  ...
  // config.yaml（保持模板默认值：name=openZetcX, yuan=openZetcX）
