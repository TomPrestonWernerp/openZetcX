...
const DEFAULT_YUAN_KEY = "openZetcX";
const LEGACY_YUAN_ALIASES = new Set(["hanako", "butter", "ming", "kong"]);

export function normalizeYuanKey(value) {
  if (value === undefined || value === null || value === "") return DEFAULT_YUAN_KEY;
  if (typeof value !== "string") {
    throw new Error(`Invalid Ta template ${JSON.stringify(value)}: expected string`);
  }
  const key = value.trim();
  if (!key || !YUAN_KEY_RE.test(key)) {
    throw new Error(`Invalid Ta template "${value}": expected a template key`);
  }
  return LEGACY_YUAN_ALIASES.has(key.toLowerCase()) ? DEFAULT_YUAN_KEY : key;
}

export function normalizeAgentConfigYuanInPlace(config) {
  if (!config || typeof config !== "object") return DEFAULT_YUAN_KEY;
  if (!config.agent || typeof config.agent !== "object" || Array.isArray(config.agent)) {
    config.agent = {};
  }
  const normalized = normalizeYuanKey(config.agent.yuan);
  config.agent.yuan = normalized;
  return normalized;
}
...
export function assertKnownYuan(productDir, value) {
  ...
  if (!isKnownYuan(productDir, key)) {
    const err = new Error(`Invalid Ta template "${key}": template not found in lib/yuan`) as Error & { code: string; statusCode: number };
...
export function getAgentConfigRepairState(config, productDir) {
  const value = config?.agent?.yuan || DEFAULT_YUAN_KEY;
  try {
    const key = normalizeYuanKey(value);
    ...
    return {
      ...
      field: "agent.yuan",
      value: key,
      message: `Invalid Ta template "${key}": template not found in lib/yuan`,
    };
  } catch (err) {
    return {
      needsRepair: true,
