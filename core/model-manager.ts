...
export class ModelManager {
  ...
  constructor({ openZetcXHome }) {
    ...
    // 新架构模块（init() 后可用）
    this.providerRegistry = new ProviderRegistry(openZetcXHome);
    this.executionRouter = null;
  }

  /** 初始化 AuthStorage + ModelRegistry + 新架构模块 */
  init() {
    this._authStorage = AuthStorage.create(path.join(this._openZetcXHome, "auth.json"));
    ...
    this._modelRegistry = createModelRegistry(
      this._authStorage,
      path.join(this._openZetcXHome, "models.json"),
    );
  ...
  get modelsJsonPath() { return path.join(this._openZetcXHome, "models.json"); }
  get authJsonPath() { return path.join(this._openZetcXHome, "auth.json"); }
  ...
  _removeApiKeyProviderAuthEntries() {
    ...
    migrateLegacyApiKeyAuthToProviders({
      openZetcXHome: this._openZetcXHome,
