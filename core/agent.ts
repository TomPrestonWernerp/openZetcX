...
import {
  assertAgentConfigPatchYuan,
  getAgentConfigRepairState,
  normalizeAgentConfigYuanInPlace,
  normalizeYuanKey,
} from "./yuan-registry.ts";
import {
...
export class Agent {
   ...
   */
  _loadConfigWithYuanRepair() {
    const config = loadConfig(this.configPath);
    const previous = config?.agent?.yuan;
    const normalized = normalizeAgentConfigYuanInPlace(config);
    if (previous !== normalized) {
      try {
        saveConfig(this.configPath, { agent: { yuan: normalized } });
        const reloaded = loadConfig(this.configPath);
        normalizeAgentConfigYuanInPlace(reloaded);
        moduleLog.log(`[yuan] repaired legacy yuan ${previous} -> ${normalized}`);
        return reloaded;
      } catch (err) {
        console.warn(`[agent] failed to persist repaired yuan for ${this.id}:`, err);
      }
    }
    return config;
  }
  ...
  async init(log: (msg?: string) => void = () => {}, sharedModels: any = {}, resolveModel = null) {
    ...
    await runCompatChecks({
      ...
      openZetcXHome: path.dirname(path.dirname(this.agentDir)),
      log,
    });
    ...
    this._browserTool = createBrowserTool(() => this._cb?.getCurrentSessionPath?.(), {
      ...
      getOpenZetcXHome: () => this._cb?.getEngine?.()?.openZetcXHome,
  ...
  updateConfig(partial, options: any = {}) {
    ...
    // 更新身份
    const isZh = String(this._config.locale || "").startsWith("zh");
    if (partial.agent?.name) this.agentName = this._config.agent?.name || "openZetcX";
  ...
  get personality() {
    ...
    const yuanType = normalizeYuanKey(this._config?.agent?.yuan);
  ...
  buildSystemPrompt( options: BuildSystemPromptOptions = {}) {
    ...
    const yuanType = normalizeYuanKey(this._config?.agent?.yuan);
    if (!this._readYuan()) throw new Error(`Cannot find Ta template "${yuanType}". Check lib/yuan/`);
    ...
    const parts = [
      isZh
        ? "你运行在 openZetcX 平台上（原名 openZetcX），由 TomPrestonWernerp 开发。项目主页：https://github.com/TomPrestonWernerp/openZetcX"
        : "You are running on the openZetcX platform (formerly openZetcX), developed by TomPrestonWernerp. Project page: https://github.com/TomPrestonWernerp/openZetcX",
    ];
