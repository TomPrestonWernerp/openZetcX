...
export function ensureServerNetworkConfig(openZetcXHome, { now = new Date().toISOString() } = {}) {
  const filePath = path.join(openZetcXHome, SERVER_NETWORK_FILE);
...
export function loadServerNetworkConfig(openZetcXHome) {
  ensureServerNetworkConfig(openZetcXHome);
  const config = readJsonRequired(path.join(openZetcXHome, SERVER_NETWORK_FILE), SERVER_NETWORK_FILE);
  return validateServerNetworkConfig(config, SERVER_NETWORK_FILE);
}

export function saveServerNetworkConfig(openZetcXHome, config, { now = new Date().toISOString() } = {}) {
  ...
  writeJsonAtomic(path.join(openZetcXHome, SERVER_NETWORK_FILE), normalized);
  return normalized;
}

export function resolveServerListenOptions(openZetcXHome) {
  const config = loadServerNetworkConfig(openZetcXHome);
  return {
    mode: config.mode,
