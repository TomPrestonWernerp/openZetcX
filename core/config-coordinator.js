...
export class ConfigCoordinator {
  /**
   * @param {object} deps
   * @param {string} deps.openZetcXHome
  ...
  async syncAndRefresh() {
    ...
    this.rebindDefaultModelFromActiveAgent();
    this.normalizeUtilityApiPreferences();
    return synced;
  }

  rebindDefaultModelFromActiveAgent({ agentId } = {}) {
    const agent = agentId ? this._d.getAgentById?.(agentId) : this._d.getAgent?.();
    const models = this._d.getModels();
    const parsed = parseModelRef(agent?.config?.models?.chat);

    if (!parsed?.id || !parsed?.provider) {
      models.defaultModel = null;
      log.warn("rebindDefaultModelFromActiveAgent: models.chat missing id/provider");
      return null;
    }
    ...
    if (!model) {
      models.defaultModel = null;
      log.warn(`rebindDefaultModelFromActiveAgent: model not available: ${parsed.provider}/${parsed.id}`);
      return null;
    }

    models.defaultModel = model;
    log.log(`default model rebound to: ${model.provider}/${model.id}`);
    return model;
  }
