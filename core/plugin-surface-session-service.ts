...
}: { openZetcXHome?: string; pluginId?: string; principalId?: string; now?: string; ttlMs?: number } = {}) {
...
export function verifyPluginSurfaceSession({
  ...
  token,
  now = new Date().toISOString(),
}: { openZetcXHome?: string; pluginId?: string; token?: string; now?: string } = {}) {
  assertNonEmpty(openZetcXHome, "openZetcXHome");
  assertNonEmpty(pluginId, "pluginId");
...
function pluginSurfaceSessionKeyPath(openZetcXHome) {
  assertNonEmpty(openZetcXHome, "openZetcXHome");
  return path.join(openZetcXHome, "security", PLUGIN_SURFACE_SESSION_KEY_FILE);
}

function signBody(openZetcXHome, body) {
  return crypto
    .createHmac("sha256", readOrCreateSessionKey(openZetcXHome))
    .update(body)
    .digest("base64url");
}

function readOrCreateSessionKey(openZetcXHome) {
  const filePath = pluginSurfaceSessionKeyPath(openZetcXHome);
  try {
