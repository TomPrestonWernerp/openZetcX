...
export function getLocalAccountSummary(openZetcXHome) {
  const { user } = loadDefaultUser(openZetcXHome);
  const auth = loadLocalUserAuth(openZetcXHome);
  const credential = findCredential(auth, user.userId);
  return sanitizeAccount(user, { passwordSet: !!credential });
}

export function updateLocalAccountProfile(openZetcXHome, {
  username,
  displayName,
  now = new Date().toISOString(),
}: { username?: string; displayName?: string; now?: string } = {}) {
  const users = loadUsers(openZetcXHome);
  ...
  user.updatedAt = now;
  users.updatedAt = now;
  writeJsonAtomic(path.join(openZetcXHome, USERS_FILE), users);
  const auth = loadLocalUserAuth(openZetcXHome);
  return sanitizeAccount(user, { passwordSet: !!findCredential(auth, user.userId) });
}

export function setLocalAccountPassword(openZetcXHome, {
  password,
  now = new Date().toISOString(),
  ...
  auth.updatedAt = now;
  writeJsonAtomic(path.join(openZetcXHome, LOCAL_USER_AUTH_FILE), auth);
  return sanitizeAccount(user, { passwordSet: true });
}

export function clearLocalAccountPassword(openZetcXHome, {
  now = new Date().toISOString(),
} = {}) {
  const { user } = loadDefaultUser(openZetcXHome);
  const auth = loadLocalUserAuth(openZetcXHome);
...
export function verifyLocalAccountPassword(openZetcXHome, { username, password }: { username?: string; password?: string } = {}) {
...
export function loadLocalUserAuth(openZetcXHome, { now = new Date().toISOString() } = {}) {
  const filePath = path.join(openZetcXHome, LOCAL_USER_AUTH_FILE);
...
function loadDefaultUser(openZetcXHome) {
  const users = loadUsers(openZetcXHome);
  return { users, user: getDefaultUser(users) };
}

function loadUsers(openZetcXHome) {
  return validateUsers(readJsonRequired(path.join(openZetcXHome, USERS_FILE), USERS_FILE), USERS_FILE);
}
