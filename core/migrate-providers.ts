 ...
 *
 * @param {string} openZetcXHome - 用户数据根目录（~/.openZetcX-dev）
 * @param {string} agentsDir  - agents 目录
 * @param {(msg: string) => void} log - 日志回调
 */
export function migrateToProvidersYaml(openZetcXHome: string, agentsDir: string, log: (msg: string) => void = () => {}) {
  const providersPath = path.join(openZetcXHome, "added-models.yaml");
  const prefsPath = path.join(openZetcXHome, "user", "preferences.json");

  // ── 文件改名迁移：providers.yaml → added-models.yaml ──
  const oldPath = path.join(openZetcXHome, "providers.yaml");
