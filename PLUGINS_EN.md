...
2. Open openZetcX → Settings → Plugins, drag the folder into the install area (or drag a .zip)
...
`minAppVersion` (optional) declares the minimum openZetcX version required to run the plugin. If the current app version is lower, the plugin will not load and its status is set to `incompatible`. All plugins should declare this field to prevent compatibility issues on older versions.
...
CLI providers must use structured argument bindings. Do not build shell command strings; openZetcX runs commands through non-shell `execFile` / `spawn` paths and collects outputs into the media task directory.
...
React plugin UIs should use `@hana/plugin-components`. It provides Button, IconButton, TextInput, Textarea, Select, Switch, SettingRow, CardShell, List, EmptyState, and related primitives that match openZetcX's current controls:
...
`HanaThemeProvider` supports three modes: `inherit` reads host CSS variables and then uses SDK fallback tokens; `hana` pins the UI to a named openZetcX theme token set; `custom` only overrides explicitly provided tokens and lets missing fields continue through the fallback chain. Components depend only on `hana-plugin-*` classes and CSS variables, not renderer internals.
...
Plugins can register background tasks so openZetcX can track and abort them. Runtime lifecycle is managed by `TaskRegistry`.
...
The "Open plugin marketplace" button in Settings -> Plugins opens a full marketplace subpage that reads `/api/plugins/marketplace`. openZetcX uses the `marketplace.json` file in this project's Git repository as the default plugin marketplace configuration, while users browse, install, enable, and disable plugins without managing marketplace sources.
...
Without either environment variable, openZetcX first tries `${HANA_HOME}/plugin-marketplace/marketplace.json` for local development. If it does not exist, openZetcX reads the default marketplace URL configured for this project's Git repository. The marketplace index shape is:
...
```json
{
  ...
  "plugins": [{
    ...
    "distribution": {
      "kind": "release",
      "packageUrl": "https://github.com/TomPrestonWernerp/openZetcX/releases/download/demo-v1.0.0/demo.zip",
      "sha256": "..."
    },
    "versions": [
      {
...
Marketplace version management uses `versions[]` as the long-term contract: each item declares `version`, that version's `compatibility.minAppVersion`, and its own `distribution`. If `versions[]` is absent, openZetcX treats the root-level `version` / `compatibility` / `distribution` as a single version entry. The client chooses the highest SemVer version compatible with the current app, while exposing `latestVersion`, `selectedVersion`, `installedVersion`, `updateAvailable`, `downgrade`, `reinstall`, `compatible`, `installAction`, and `canInstall` for UI state.

If the installed version is newer than the highest compatible marketplace version, the action is marked as `downgrade` and install requires explicit `allowDowngrade: true`. Drag-and-drop / local path installs also reject implicit downgrades. Updates back up the previous plugin directory under `${HANA_HOME}/plugin-backups/<pluginId>/`; if the new version fails to load, openZetcX restores and reloads the old directory. Successful installs are recorded in `${HANA_HOME}/plugin-installs.json` with source, version, release URL, and sha256 so later marketplace state is explicit.
...
openZetcX supports multiple sessions and multiple agents running in parallel. Keep the following in mind when developing plugins:
