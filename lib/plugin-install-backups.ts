...
}: { openZetcXHome?: string; pluginId?: string; pluginDir?: string; version?: string; maxBackups?: number } = {}) {
  if (!openZetcXHome || !pluginId || !pluginDir || !fs.existsSync(pluginDir)) return null;
  const backupRoot = path.join(openZetcXHome, "plugin-backups", safeSegment(pluginId));
