...
export const DEFAULT_OFFICIAL_PLUGIN_MARKETPLACE_URL = "https://raw.githubusercontent.com/TomPrestonWernerp/openZetcX/main/marketplace.json";
...
export function createDefaultPluginMarketplace({ hanakoHome, env = process.env, fetchImpl } = {}) {
  ...
  const bundledIndex = hanakoHome ? fromRoot("marketplace.json") : null;
  const indexPath = envPath
    || (!envUrl && localIndex && fs.existsSync(localIndex) ? localIndex : null)
    || (!envUrl && bundledIndex && fs.existsSync(bundledIndex) ? bundledIndex : null);
  return new PluginMarketplace({
    indexPath,
    indexUrl: envUrl || (indexPath ? null : DEFAULT_OFFICIAL_PLUGIN_MARKETPLACE_URL),
    fetchImpl,
  });
}
