/**
 * channel-store.js — 集群 MD 文件的读写层
 *
 * 集群 = 一个 MD 文件，frontmatter 记元数据，正文是消息流。
 * 每个 agent 的 channels.md 记录她加入了哪些集群、读到哪了（bookmark = 时间戳）。
 *
 * 设计原则：文件就是一切，不引入数据库。
 */
...
/**
 * 解析集群 MD 文件，提取 frontmatter 和消息列表
 * @param {string} content - 集群 MD 文件的全文
...
/**
 * 创建集群 MD 文件
 * @param {string} channelsDir - 集群目录路径
 * @param {object} opts
 * @param {string} [opts.id] - channel ID（不传则自动生成）
 * @param {string} [opts.name] - 集群显示名
 * @param {string} [opts.description] - 集群描述
 * @param {string[]} opts.members - 成员列表
 * @param {string} [opts.intro] - 集群介绍（作为第一条系统消息）
...
/**
 * 向集群追加一条消息
 * @param {string} filePath - 集群 MD 文件路径
...
/**
 * 获取集群的元数据（id, name, description, members 等）
 * @param {string} filePath - 集群 MD 文件路径
 * @returns {object}
 */
...
/**
 * 读取集群文件，修改 frontmatter 后重写，保留消息部分不变
 *
...
/**
 * 更新 agent 的某个集群 bookmark
 * @param {string} channelsMdPath - agent 的 channels.md 路径
 * @param {string} channelName - 集群名
...
/**
 * 向 agent 的 channels.md 添加一个新集群条目
 * @param {string} channelsMdPath - agent 的 channels.md 路径
 * @param {string} channelName - 集群名
 */
...
/**
 * 从 agent 的 channels.md 移除某个集群条目
 * @param {string} channelsMdPath - agent 的 channels.md 路径
 * @param {string} channelName - 要移除的集群名
 */
