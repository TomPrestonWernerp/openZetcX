...
export class MountProvider {
  ...
  constructor({ openZetcXHome, studioId, localFsProviderFactory }: Options) {
    if (!openZetcXHome) throw new Error("openZetcXHome is required");
  ...
  mountForRef(ref: ResourceRef) {
    ...
    let registry;
    try {
      registry = loadStudioMountRegistry(this.openZetcXHome);
    } catch (err) {
