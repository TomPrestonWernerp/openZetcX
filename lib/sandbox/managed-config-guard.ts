...
export function isManagedAgentConfigPath(filePath, { openZetcXHome }: { openZetcXHome?: string } = {}) {
  if (!filePath || !openZetcXHome) return false;

  const target = resolveExistingOrNearest(filePath);
  const home = resolveExistingOrNearest(openZetcXHome);
...
export function createManagedConfigWriteGuard({ openZetcXHome }: { openZetcXHome?: string } = {}) {
  return (absolutePath, operation) => {
    if (operation !== "write" && operation !== "delete") return { allowed: true };
    if (!isManagedAgentConfigPath(absolutePath, { openZetcXHome })) return { allowed: true };
    return {
      allowed: false,
