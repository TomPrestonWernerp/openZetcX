...
export function externalReadPathsFromSessionFiles(files = [], { workspaceRoots = [], openZetcXHome }: { workspaceRoots?: any[]; openZetcXHome?: any } = {}) {
  const normalizedWorkspaceRoots = uniqueNormalized(workspaceRoots);
  const normalizedHome = openZetcXHome ? normalizeExistingOrResolved(openZetcXHome) : null;
  const out = [];
