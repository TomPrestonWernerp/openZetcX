...
export class PathGuard {
  ...
  constructor(policy) {
    ...
    const home = policy.openZetcXHome || policy["hanako" + "Home"];
    this.openZetcXHome = this._resolveReal(home) || path.resolve(home);
  ...
  getAccessLevel(rawPath) {
    ...
    // 1. BLOCKED 文件（openZetcXHome 根）
    for (const f of BLOCKED_FILES) {
      if (resolved === path.join(this.openZetcXHome, f)) return AccessLevel.BLOCKED;
    }

    // 2. BLOCKED 目录
    for (const d of BLOCKED_DIRS) {
      if (this._isInside(resolved, path.join(this.openZetcXHome, d))) {
    ...
    // 8. openZetcXHome 内未匹配 → 遵守 read-all 契约：非敏感路径可读，写仍需显式白名单。
    if (this._isInside(resolved, this.openZetcXHome)) {
