...
function findGitRuntime({ env = process.env, bundledOnly = false } = {}) {
  ...
  throw new Error(
    "[win32-exec] No usable git.exe found. Install Git for Windows or rebuild openZetcX with bundled PortableGit."
  );
}
...
function withWin32SandboxRuntimeEnv(baseEnv, sandbox) {
  ...
  if (!sandboxIsEnabled(sandbox) || !sandbox?.openZetcXHome) return env;

  const root = joinRuntimePath(sandbox.openZetcXHome, ".ephemeral", WIN32_SANDBOX_ENV_DIR);
...
function redactWin32DiagnosticText(value: any, { sandbox, env }: { sandbox?: any; env?: any } = {}) {
  ...
  const replacements = [
    [sandbox?.openZetcXHome, "<HANA_HOME>"],
...
function emitWin32RuntimeFailureDiagnostic(onData, {
  ...
  const lines = [
    ...
    sandbox?.openZetcXHome ? "HANA_HOME: <HANA_HOME>" : null,
...
function prepareRuntimeForSandbox(runtimeInfo, sandbox, kind) {
  if (!sandboxIsEnabled(sandbox) || !sandbox?.openZetcXHome) return runtimeInfo;
  return prepareSandboxRuntime(runtimeInfo, {
    openZetcXHome: sandbox.openZetcXHome,
    kind,
  });
}
