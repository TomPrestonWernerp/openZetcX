 ...
 */
export function deriveSandboxPolicy({ agentDir, workspace, workspaceFolders = [], hanakoHome, openZetcXHome, mode }) {
  if (mode === "full-access") {
  ...
  return {
    ...
    writablePaths: [
      ...
      ...READ_WRITE_HOME_DIRS.map((d) => path.join(homeDir, d)),
      ...workspaceRoots,
    ].filter(Boolean),
    ...
    readablePaths: [
      ...
      ...READ_ONLY_HOME_DIRS.map((d) => path.join(homeDir, d)),
    ].filter(Boolean),

    // OS 沙盒用：拒绝读取（文件 + 目录）
    denyReadPaths: [
      ...BLOCKED_FILES.map((f) => path.join(homeDir, f)),
      ...BLOCKED_DIRS.map((d) => path.join(homeDir, d)),
    ],
    ...
    protectedPaths: [
      ...
      path.join(homeDir, "session-files"),
    ].filter(Boolean),
  };
}
