 ...
 * @param {string} opts.openZetcXHome
 * @param {"standard"|"full-access"} opts.mode
 * @returns {object} policy
 */
export function deriveSandboxPolicy({ agentDir, workspace, workspaceFolders = [], openZetcXHome, mode }) {
  if (mode === "full-access") {
  ...
  return {
    ...
    writablePaths: [
      ...
      ...READ_WRITE_HOME_DIRS.map((d) => path.join(openZetcXHome, d)),
      ...workspaceRoots,
    ].filter(Boolean),
    ...
    readablePaths: [
      ...
      ...READ_ONLY_HOME_DIRS.map((d) => path.join(openZetcXHome, d)),
    ].filter(Boolean),

    // OS 沙盒用：拒绝读取（文件 + 目录）
    denyReadPaths: [
      ...BLOCKED_FILES.map((f) => path.join(openZetcXHome, f)),
      ...BLOCKED_DIRS.map((d) => path.join(openZetcXHome, d)),
    ],
