...
const LEGACY_PROFILE_PREFIX = "com.openZetcX.sandbox.";
...
export function isLegacyWin32SandboxProfileName(name) {
  return typeof name === "string" && /^com\.openZetcX\.sandbox\.\d+\.\d+$/i.test(name);
}
...
export function collectWin32LegacySandboxMigrationTargets({
  ...
  if (openZetcXHome) {
    push(openZetcXHome);
    push(joinWin32Aware(openZetcXHome, ".ephemeral"));
    push(joinWin32Aware(openZetcXHome, "agents"));
    push(joinWin32Aware(openZetcXHome, "session-files"));
    push(joinWin32Aware(openZetcXHome, "uploads"));
  }
...
function defaultMigrationMarkerPath(openZetcXHome) {
  return openZetcXHome
    ? path.join(openZetcXHome, "user", "win32-sandbox-migration-v3.json")
    : null;
}

function defaultCleanupMarkerPath(openZetcXHome) {
  return openZetcXHome
    ? path.join(openZetcXHome, "user", "win32-sandbox-cleanup-v4.json")
    : null;
}
...
export async function runWin32LegacySandboxMigration({
  ...
  const completionMarkerPath = cleanup && !disableMarker
    ? (markerPath || defaultMigrationMarkerPath(openZetcXHome))
    : null;
...
export async function runWin32LegacySandboxRootCleanup({
  ...
  const marker = markerPath || defaultCleanupMarkerPath(openZetcXHome);
...
export class Win32LegacySandboxCleanupQueue {
  ...
  constructor({
    ...
    this.markerPath = markerPath || defaultCleanupMarkerPath(openZetcXHome);
    this.env = env;
    this.spawn = spawn;
