...
function ensureCachedRuntimeRoot({ sourceRoot, primaryPath, openZetcXHome, kind }) {
  if (!openZetcXHome) {
  ...
  const cacheRoot = sandboxRuntimeCacheRoot(openZetcXHome);
...
export function sandboxRuntimeCacheRoot(openZetcXHome) {
  if (!openZetcXHome) throw new Error("[win32-sandbox] HANA_HOME is required for sandbox runtime cache.");
  return joinRuntimePath(openZetcXHome, ".ephemeral", CACHE_DIR);
}

export function prepareSandboxRuntime(runtimeInfo, { openZetcXHome, kind }) {
  ...
  const targetRoot = ensureCachedRuntimeRoot({ sourceRoot, primaryPath, openZetcXHome, kind });
