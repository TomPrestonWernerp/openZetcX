...
const DEFAULT_CARD_YUAN = "openZetcX";
const LEGACY_CARD_YUAN = new Set(["hanako", "butter", "ming", "kong"]);
const VALID_YUAN = new Set([DEFAULT_CARD_YUAN]);
...
function normalizeCardYuan(value) {
  const key = trimString(value);
  if (!key) return DEFAULT_CARD_YUAN;
  if (VALID_YUAN.has(key)) return key;
  if (LEGACY_CARD_YUAN.has(key.toLowerCase())) return DEFAULT_CARD_YUAN;
  return DEFAULT_CARD_YUAN;
}
...
function normalizeAgent(card) {
  ...
  const yuan = normalizeCardYuan(agent.yuan || card?.yuan);
...
function defaultAvatarForYuan(_yuan) {
  return fromRoot("desktop", "src", "assets", "openZetcX.png");
}

function defaultYuanIconForYuan(_yuan) {
  return fromRoot("desktop", "src", "assets", "openZetcX.png");
}
...
function ensurePreviewAssets(plan) {
  const yuan = normalizeCardYuan(plan.agent?.yuan);
...
export function createCharacterCardService(engine) {
  const stagingRoot = path.join(engine.openZetcXHome, ".ephemeral", "character-card-imports");
  ...
  function readAgentExportSource(agentId) {
    ...
    const yuan = normalizeCardYuan(config?.agent?.yuan);
  ...
  async function exportAgentPackage(agentId, options = {}) {
    ...
    const token = randomToken();
    const exportRoot = path.join(engine.openZetcXHome, ".ephemeral", "character-card-exports", token);
