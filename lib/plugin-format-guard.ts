...
function createOpenClawIssue({ marker, id, name, version }) {
  return {
    ...
    name,
    version,
    message: `This package looks like an OpenClaw plugin (${marker}). Hana cannot install OpenClaw plugin zips directly. Use a openZetcX plugin package with manifest.json, or port the plugin to Hana's plugin SDK.`,
  };
}
