...
export function createArtifactTool({ getopenZetcXHome, registerSessionFile, getSessionPath } = {}) {
  return {
...
async function persistArtifactFile({
  ...
  const openZetcXHome = getopenZetcXHome?.() || null;
  if (!openZetcXHome) throw new Error("create_artifact requires openZetcXHome to persist generated artifacts");
  ...
  const dir = sessionFilesCacheDir(openZetcXHome, sessionPath);
