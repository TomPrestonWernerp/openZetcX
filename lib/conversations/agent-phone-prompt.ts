...
export const AGENT_PHONE_REFLECTION_GUIDES: Readonly<Record<string, { zhName: string; enName: string; tag: string }>> = Object.freeze({
  openZetcX: { zhName: "TA", enName: "TA", tag: "mood" },
});
...
export function resolveAgentPhoneReflectionGuide({ agentId, agent = null, agentsDir = null }: { agentId?: any; agent?: any; agentsDir?: any } = {}) {
  try {
    ...
    const key = cfg?.agent?.yuan || null;
    if (!key) return null;
    return AGENT_PHONE_REFLECTION_GUIDES[key] || AGENT_PHONE_REFLECTION_GUIDES.openZetcX;
  } catch {
    return null;
  }
}
