/**
 * heartbeat.js — 日常巡检 + 便签目录扫描
 *
 * 让 agent 从被动应答变成主动行动的关键机制。
 * 两个阶段：
 *   Phase 1: 工作台文件变化检测
 *   Phase 2: 便签扫描（根目录 + 一级子目录的 jian.md，指纹比对后隔离执行）
 *
...
function buildHeartbeatContext({ deskChanged, changedFiles, overwatch, agentName, isZh, patrolLog }) {
  ...
  const activityDir = `${HEARTBEAT_ACTIVITY_DIR}/${agentName || "openZetcX"}`;
  parts.push("---");
...
function buildJianPrompt({ dirPath, jianContent, files, jianChanged, filesChanged, isZh }) {
  ...
  parts.push(isZh
    ? [
        ...
        "   <!-- /exec-log -->",
        "   ```",
        "   如果已有 `<!-- exec-log -->` 标签，在标签内追加新行；没有则在便签末尾新建。",
 ...
 * @param {(prompt: string, cwd: string) => Promise<void>} [opts.onJianBeat] - 便签巡检回调（带 cwd）
...
export function createHeartbeat({
  ...
  async function _doBeat() {
    try {
      ...
      if (jianDirs.length > 0) summaryParts.push(isZh ? `便签: ${jianDirs.length} 目录, ${jianChanges.length} 变化` : `jian: ${jianDirs.length} dirs, ${jianChanges.length} changed`);
      ...
      {
        const agentName = getAgentName?.() || "openZetcX";
        // 读取巡检日志（截断）
        ...
        console.log(`${tag}  Phase 1: 工作台巡检 (${prompt.length} chars, ${deskChanged ? "有变化" : "无变化"})`);
        ...
        try {
          ...
          console.error(`[heartbeat] Phase 1 error: ${err.message}`);
          debugLog()?.error("heartbeat", `phase 1 error: ${err.message}`);
          devlog(`Phase 1 错误: ${err.message}`, "error");
        } finally {
          clearTimeout(timer);
        }
      }
  ...
  async function _processJianChanges(changes, tag) {
    ...
    for (const dir of changes) {
      ...
      console.log(`${tag}  Phase 2: 便签 [${label}] 有变化，执行中...`);
      devlog(`便签 [${label}] 有变化，执行中...`);
      ...
      try {
        {
          ...
          try {
            await Promise.race([
              ...
              new Promise((_, reject) => { timer = setTimeout(() => reject(new Error(isZh ? `便签 [${label}] 执行超时 (5min)` : `Jian [${label}] timed out (5min)`)), BEAT_TIMEOUT); }),
            ]);
          } finally {
            clearTimeout(timer);
          }
        ...
        devlog(`便签 [${label}] 执行完成`);
      } catch (err) {
        devlog(`便签 [${label}] 执行失败: ${err.message}`, "error");
      }
    }
  }
