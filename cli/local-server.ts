...
export function resolveCliHanaHome(env = process.env) {
  return resolveHomePath(env.HANA_HOME || path.join(os.homedir(), ".openZetcX"));
}

function resolveHomePath(value) {
  const raw = String(value || "").trim();
  if (!raw) return path.join(os.homedir(), ".openZetcX");
...
export function readLocalServerInfo({ hanaHome = resolveCliHanaHome(), checkProcess = true } = {}) {
  ...
  if (!fs.existsSync(filePath)) {
    return {
      ...
      filePath,
      message: `No running openZetcX Server was found at ${filePath}`,
    };
  }
  ...
  if (checkProcess && Number.isInteger(info.pid) && !isProcessAlive(info.pid)) {
    return {
      ...
      filePath,
      message: `openZetcX Server process ${info.pid} is no longer running`,
    };
  }
