...
export function randomWelcome(agentName?: string, _yuan?: string): string {
  const s = useStore.getState();
  const name = agentName || s.agentName;
  const yuanMsgs = tr('yuan.welcome.openZetcX');
...
export function yuanPlaceholder(_yuan?: string): string {
  const yuanPh = tr('yuan.placeholder.openZetcX');
