...
function defaultChannelWorkspaceParentRoot(s: ReturnType<typeof useStore.getState>): string {
  return normalizeFolder(s.homeFolder) || normalizeFolder(s.selectedFolder) || '';
}
...
function defaultChannelWorkspaceBaseRoot(s: ReturnType<typeof useStore.getState>): string {
  ...
  return parentRoot ? joinWorkspacePath(parentRoot, WORKSPACE_OUTPUT_ROOT_DIRNAME) : '';
}
...
function resolveChannelWorkspaceRootValue(value: unknown, s: ReturnType<typeof useStore.getState>, mode: 'base' | 'channel'): string | null {
  ...
  if (isAbsoluteWorkspacePath(trimmed)) return normalizeFolder(trimmed);
  return normalizeFolder(resolveRelativeChannelWorkspaceRoot(trimmed, s, mode));
}

function currentChannelWorkspaceRoot(s: ReturnType<typeof useStore.getState>): string | null {
  const channelId = typeof s.currentChannel === 'string' && s.currentChannel.trim()
    ? s.currentChannel.trim()
    : '';
  if (!channelId || s.channelIsDM === true) return null;
  const channel = Array.isArray(s.channels)
    ? s.channels.find((item: any) => item?.id === channelId)
    : null;
  const workspace = s.channelWorkspaceById?.[channelId];
  const candidates = [
    workspace?.workspaceRoot,
    channel?.workspaceRoot,
  ];
  for (const candidate of candidates) {
    const normalized = resolveChannelWorkspaceRootValue(candidate, s, 'channel');
    if (normalized) return normalized;
  }
  return null;
}

function canonicalPathKey(value: string | null | undefined): string {
  return normalizeFolder(value)?.replace(/\\/g, '/').replace(/\/+$/g, '').toLowerCase() || '';
}

function isSameOrInsidePath(candidate: string | null | undefined, root: string | null | undefined): boolean {
  const candidateKey = canonicalPathKey(candidate);
  const rootKey = canonicalPathKey(root);
  return !!candidateKey && !!rootKey && (candidateKey === rootKey || candidateKey.startsWith(`${rootKey}/`));
}

function knownChannelWorkspaceRoots(s: ReturnType<typeof useStore.getState>): string[] {
  const roots: string[] = [];
  const pushRoot = (value: unknown, mode: 'base' | 'channel' = 'channel') => {
    const normalized = resolveChannelWorkspaceRootValue(value, s, mode);
    if (normalized && !roots.some(root => canonicalPathKey(root) === canonicalPathKey(normalized))) {
      roots.push(normalized);
    }
  };
  pushRoot(s.channelWorkspaceRoot, 'base');
  if (s.channelWorkspaceById && typeof s.channelWorkspaceById === 'object') {
    for (const workspace of Object.values(s.channelWorkspaceById)) {
      pushRoot((workspace as any)?.workspaceRoot);
    }
  }
  if (Array.isArray(s.channels)) {
    for (const channel of s.channels as any[]) {
      if (!channel?.isDM) pushRoot(channel?.workspaceRoot);
    }
  }
  pushRoot(currentChannelWorkspaceRoot(s));
  return roots;
}

function isKnownChannelWorkspaceRoot(s: ReturnType<typeof useStore.getState>, root: string | null | undefined): boolean {
  return knownChannelWorkspaceRoots(s).some(channelRoot => isSameOrInsidePath(root, channelRoot));
}

function isChannelDeskScope(s: ReturnType<typeof useStore.getState>, rootOverride?: string | null): boolean {
  if (rootOverride !== undefined && isKnownChannelWorkspaceRoot(s, rootOverride)) return true;
  if (!s.currentChannel || s.channelIsDM === true) return false;
  if (s.currentTab === 'channels') return true;
  const channelRoot = currentChannelWorkspaceRoot(s);
  const deskRoot = normalizeFolder(s.deskBasePath);
  return !!channelRoot && !!deskRoot && channelRoot === deskRoot;
}

function selectedDeskAgentId(s: ReturnType<typeof useStore.getState>, rootOverride?: string | null): string | null {
  if (isChannelDeskScope(s, rootOverride)) return null;
  return typeof s.selectedAgentId === 'string' && s.selectedAgentId.trim()
    ? s.selectedAgentId.trim()
    : null;
}

function addSelectedDeskAgentParam(params: URLSearchParams, s: ReturnType<typeof useStore.getState>, rootOverride?: string | null): void {
  const agentId = selectedDeskAgentId(s, rootOverride);
  if (agentId) params.set('agentId', agentId);
}

function selectedDeskAgentBody(s: ReturnType<typeof useStore.getState>, rootOverride?: string | null): { agentId?: string } {
  const agentId = selectedDeskAgentId(s, rootOverride);
...
export async function loadDeskFiles(subdir?: string, overrideDir?: string | null, overrideMountId?: string | null): Promise<void> {
  ...
  try {
    ...
    if (!mountId) {
      addSelectedDeskAgentParam(params, s, dir);
    }
    const curPath = '';
...
export async function loadDeskTreeFiles(subdir = '', options: { force?: boolean; overrideDir?: string | null; overrideMountId?: string | null } = {}): Promise<boolean> {
  ...
  try {
    ...
    if (!mountId) addSelectedDeskAgentParam(params, s, dir);
    const qs = params.toString() ? `?${params}` : '';
    const res = await hanaFetch(`${mountId ? '/api/workbench/files' : '/api/desk/files'}${qs}`);
    const data = await res.json();
...
export async function loadJianContent(): Promise<void> {
  ...
  try {
    ...
    if (mountId) {
      ...
      addSelectedDeskAgentParam(params, s, s.deskBasePath);
    } else {
      addSelectedDeskAgentParam(params, s, s.deskBasePath);
    }
    const qs = params.toString() ? `?${params}` : '';
  ...
  try {
    ...
    await hanaFetch(mountId ? '/api/workbench/actions' : '/api/desk/jian', {
      ...
      body: JSON.stringify(mountId
        ...
        : { ...selectedDeskAgentBody(s, s.deskBasePath), dir: s.deskBasePath || undefined, subdir: '', content: text }),
    });
    ...
    if (activeMountId) {
      ...
      addSelectedDeskAgentParam(params, st2, st2.deskBasePath);
    }
    const qs = params.toString() ? `?${params}` : '';
...
export async function deskUploadFiles(paths: string[]): Promise<void> {
  ...
  try {
    const res = await hanaFetch('/api/desk/files', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...selectedDeskAgentBody(s, s.deskBasePath), action: 'upload', dir: s.deskBasePath || undefined, subdir: '', paths }),
    });
    const data = await res.json();
    if (data.files) {
      const st = useStore.getState();
...
export async function deskUploadFilesToSubdir(paths: string[], subdir: string): Promise<void> {
  ...
  try {
    const res = await hanaFetch('/api/desk/files', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...selectedDeskAgentBody(s, s.deskBasePath), action: 'upload', dir: s.deskBasePath || undefined, subdir: normalizedSubdir, paths }),
    });
    const data = await res.json();
    if (data.files) {
      const st = useStore.getState();
...
export async function deskUploadBrowserFilesToSubdir(files: File[], subdir: string): Promise<boolean> {
  ...
  try {
    ...
    const res = await hanaFetch(mountId ? '/api/workbench/upload' : '/api/desk/files', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(mountId
        ? {
        ...
        : {
            ...selectedDeskAgentBody(s, s.deskBasePath),
            action: 'upload',
    ...
    if (data.error || data.ok === false) {
      console.error('[jian-desk] upload error:', data.error || data.results);
      return false;
    }
...
export async function deskMoveFiles(names: string[], destFolder: string): Promise<void> {
  ...
  try {
    ...
    const res = await hanaFetch('/api/desk/files', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...selectedDeskAgentBody(s, s.deskBasePath), action: 'move', dir: s.deskBasePath || undefined, subdir: '', names, destFolder }),
    });
    const data = await res.json();
    if (data.files) {
      const st = useStore.getState();
  ...
  try {
    const res = await hanaFetch(mountId ? '/api/workbench/actions' : '/api/desk/files', {
      ...
      body: JSON.stringify(mountId
        ...
        : { ...selectedDeskAgentBody(s, s.deskBasePath), action: 'remove', dir: s.deskBasePath || undefined, subdir: '', name }),
    });
    const data = await res.json();
    if (data.files) {
      const st = useStore.getState();
  ...
  try {
    const res = await hanaFetch(mountId ? '/api/workbench/actions' : '/api/desk/files', {
      ...
      body: JSON.stringify(mountId
        ...
        : { ...selectedDeskAgentBody(s, s.deskBasePath), action: 'rename', dir: s.deskBasePath || undefined, subdir: '', oldName, newName }),
    });
    const data = await res.json();
