...
import type { Session, SessionPermissionMode } from '../types';
...
function sessionHasVisibleContent(session: Session): boolean {
  ...
  return !!title || !!firstMessage || messageCount > 0;
}

function sessionIsActiveShell(session: Session, state: Record<string, any>): boolean {
  return session.path === state.currentSessionPath || session.path === state.pendingSessionSwitchPath;
}

function filterVisibleSessions(sessions: Session[], state: Record<string, any>): Session[] {
  return sessions.filter(session => (
    sessionHasVisibleContent(session) || sessionIsActiveShell(session, state)
  ));
}
...
export async function loadSessions(): Promise<void> {
  try {
    ...
    const data = await res.json();
    const sessions = filterVisibleSessions(data || [], useStore.getState() as Record<string, any>);
...
export async function switchSession(path: string): Promise<void> {
  ...
  try {
    ...
    if (switchedAgent) {
      ...
      agentPatch.agentYuan = ag?.yuan || 'openZetcX';
