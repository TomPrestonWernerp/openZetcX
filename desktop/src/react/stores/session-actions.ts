...
function removeSessionLocally(path: string): void {
  clearSessionRuntimeCaches(path);
  useStore.setState((s: Record<string, any>) => ({
    sessions: (s.sessions || []).filter((session: { path?: string }) => session.path !== path),
    currentSessionPath: s.currentSessionPath === path ? null : s.currentSessionPath,
    pendingSessionSwitchPath: s.pendingSessionSwitchPath === path ? null : s.pendingSessionSwitchPath,
  }));
}
...
export async function switchSession(path: string): Promise<void> {
  ...
  try {
    const res = await fetch(hanaUrl('/api/sessions/switch'), {
      method: 'POST',
    ...
    if (!res.ok || data.error) {
      if (data.staleSession || res.status === 404 || res.status === 410) {
        removeSessionLocally(path);
        await loadSessions();
        return;
      }
      console.error('[session] switch failed:', data.error);
      useStore.setState({ pendingSessionSwitchPath: null });
      showSessionSwitchError(path, data.error || `${res.status} ${res.statusText}`);
      return;
    }
