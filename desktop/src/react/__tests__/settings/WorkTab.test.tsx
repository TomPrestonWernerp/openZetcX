...
describe('WorkTab workspace persistence', () => {
  ...
  it('saves the selected group workspace under channel workspaces', async () => {
    useStore.setState({
      currentChannel: 'ch1',
      channels: [{
        id: 'ch1',
    ...
    autoSaveConfigMock.mockResolvedValueOnce(true as never);
    const { WorkTab } = await import('../../settings/tabs/WorkTab');

    render(<WorkTab />);

    fireEvent.click(screen.getByPlaceholderText('settings.work.channelWorkspaceRootPlaceholder'));

    await waitFor(() => {
      expect(autoSaveConfigMock).toHaveBeenCalledWith({
        channel: {
          workspaces: {
            ch1: { workspace_root: '/new-home' },
          },
        },
      });
    });
    expect(useStore.getState().channelWorkspaceById.ch1.workspaceRoot).toBe('/new-home');
    expect(useStore.getState().channels.find((channel: any) => channel.id === 'ch1')?.workspaceRoot).toBe('/new-home');
  });

  it('shows the persisted channel workspace when settings do not have an explicit entry yet', async () => {
    useStore.setState({
      currentChannel: 'ch1',
      channels: [{
        id: 'ch1',
        ...
        lastTimestamp: '',
        newMessageCount: 0,
        workspaceRoot: '/persisted-channel-root',
        isDM: false,
      }],
    });
    mockState.settingsConfig = {
    ...
    expect(screen.getByDisplayValue('/persisted-channel-root')).toBeTruthy();
  });
