...
const tMap: Record<string, string> = {
  ...
  'rightWorkspace.jian.collapse': '收起便签',
  'rightWorkspace.jian.expand': '展开便签',
...
function resetStore(items: ChatListItem[] = []) {
  useStore.setState({
    ...
    deskFiles: [],
    deskJianContent: '',
    agents: [{ id: 'hana', name: 'Hana', yuan: 'openZetcX', hasAvatar: false }],
    currentAgentId: 'hana',
    selectedFolder: null,
    homeFolder: null,
    jianView: 'desk',
...
describe('RightWorkspacePanel', () => {
  ...
  it('collapses and expands the Jian drawer without unmounting its editor state', () => {
    ...
    const drawer = screen.getByRole('region', { name: '便签' });
    ...
    expect(screen.getByRole('button', { name: '展开便签' })).toHaveAttribute('aria-expanded', 'false');
    expect(screen.getByPlaceholderText('写点什么...')).toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: '展开便签' }));

    expect(drawer).toHaveAttribute('data-open', 'true');
    expect(screen.getByRole('button', { name: '收起便签' })).toHaveAttribute('aria-expanded', 'true');
  });
});
