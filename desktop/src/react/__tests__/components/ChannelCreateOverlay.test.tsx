...
describe('ChannelCreateOverlay', () => {
  beforeEach(() => {
    ...
    vi.stubGlobal('platform', {
      selectFolder: vi.fn(async () => '/workspace/group'),
    });
  });
  ...
  it('keeps the overlay open and shows the backend rejection message', async () => {
    ...
    fireEvent.click(screen.getByTitle('channel.createWorkspaceBrowse'));
    await waitFor(() => {
      expect(screen.getByDisplayValue('/workspace/group')).toBeTruthy();
    });
  ...
  it('submits without a selected workspace directory so the channel can derive one', async () => {
    vi.mocked(createChannel).mockResolvedValueOnce(null);
    ...
    expect((screen.getByRole('button', { name: 'channel.createConfirm' }) as HTMLButtonElement).disabled).toBe(false);
    fireEvent.click(screen.getByRole('button', { name: 'channel.createConfirm' }));

    await waitFor(() => {
      expect(createChannel).toHaveBeenCalledWith('mixed', ['alice', 'bob'], undefined, undefined);
    });
  });

  it('submits the selected workspace directory when one is chosen', async () => {
    vi.mocked(createChannel).mockResolvedValueOnce(null);
    ...
    await waitFor(() => {
      expect(createChannel).toHaveBeenCalledWith('mixed', ['alice', 'bob'], undefined, '/workspace/group');
    });
  });
});
