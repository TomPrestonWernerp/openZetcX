...
const mocks = vi.hoisted(() => ({
  ...
  resourceWatchKey: (ref: any) => ref.kind === 'mount'
    ...
    : `local-file:${ref.path}`,
  openWorkbenchFilePreview: vi.fn(async () => {}),
}));
...
vi.mock('../../utils/remote-file-preview', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../utils/remote-file-preview')>();
  return {
    ...actual,
    openWorkbenchFilePreview: mocks.openWorkbenchFilePreview,
  };
});
...
describe('DeskSection workspace watching', () => {
  ...
  it('previews local workspace files from the tree on single click', async () => {
    useStore.setState({
      deskCurrentPath: '',
      deskTreeFilesByPath: {
        '': [{ name: 'openZetcX.docx', isDir: false }],
      },
      deskExpandedPaths: [],
      deskWorkspaceMountId: null,
      deskWorkspaceNativeRoot: null,
    ...
    render(<DeskSection />);
    fireEvent.click(screen.getByRole('treeitem', { name: /openZetcX.docx/ }));

    expect(mocks.openWorkbenchFilePreview).toHaveBeenCalledWith({
      file: { name: 'openZetcX.docx', isDir: false },
      subdir: '',
      mountId: null,
      localRootPath: '/tmp/hana-desk',
      nativeRootPath: '/tmp/hana-desk',
    });
  });
