...
function mockPermissionDefault(mode = 'ask') {
  ...
  describe('loadSessions 首次自动切换', () => {
    ...
    it('keeps sidebar sessions from every agent while preserving a pending target', async () => {
      Object.assign(mockState, {
        currentAgentId: 'open',
        currentSessionPath: null,
        pendingSessionSwitchPath: '/agents/developer/sessions/current.jsonl',
        pendingNewSession: false,
      });
      mockFetch.mockResolvedValueOnce(jsonResponse([
        { path: '/agents/open/sessions/keep.jsonl', agentId: 'open', firstMessage: 'hello' },
        { path: '/agents/developer/sessions/hide.jsonl', agentId: 'developer', firstMessage: 'internal' },
        { path: '/agents/developer/sessions/current.jsonl', agentId: 'developer', firstMessage: 'opening' },
      ]));
    ...
    it('filters empty placeholder sessions unless they are current or pending', async () => {
      Object.assign(mockState, {
        currentAgentId: 'open',
        currentSessionPath: '/agents/open/sessions/current.jsonl',
        pendingSessionSwitchPath: null,
        pendingNewSession: false,
      });
      mockFetch.mockResolvedValueOnce(jsonResponse([
        { path: '/agents/open/sessions/empty.jsonl', agentId: 'open', title: null, firstMessage: '', messageCount: 0 },
        { path: '/agents/open/sessions/current.jsonl', agentId: 'open', title: null, firstMessage: '', messageCount: 0 },
        { path: '/agents/open/sessions/real.jsonl', agentId: 'open', title: '测试', firstMessage: '', messageCount: 0 },
      ]));
  ...
  describe('archiveSession 按 path 清缓存', () => {
    ...
    it('归档当前 session 时在 currentSessionPath 置空前按该 path 清理缓存', async () => {
      ...
      mockFetch.mockResolvedValueOnce(jsonResponse({ ok: true }));
      mockFetch.mockResolvedValueOnce(jsonResponse([{ path: '/other', firstMessage: 'other' }]));
      mockFetch.mockResolvedValueOnce(jsonResponse({
        agentId: null,
        cwd: '/workspace-other',
        currentModelId: null,
