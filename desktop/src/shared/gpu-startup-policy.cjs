...
function getGpuStartupStatePath(openZetcXHome) {
  return path.join(openZetcXHome, STATE_FILE);
}

function getPreferencesPath(openZetcXHome) {
  return path.join(openZetcXHome, PREFERENCES_FILE);
}

function readState(openZetcXHome) {
  return readJson(getGpuStartupStatePath(openZetcXHome), { version: STATE_VERSION });
}

function writeState(openZetcXHome, state) {
  writeJson(getGpuStartupStatePath(openZetcXHome), {
    ...state,
    version: STATE_VERSION,
  });
}

function readPreferences(openZetcXHome) {
  return readJson(getPreferencesPath(openZetcXHome), {});
}

function writePreferences(openZetcXHome, prefs) {
  writeJson(getPreferencesPath(openZetcXHome), prefs);
}
...
function writeAutoGpuMode(openZetcXHome, mode, {
  reason,
  previousMode,
  previousStartup,
  now,
  ...
  const state = readState(openZetcXHome);
  writeState(openZetcXHome, {
    ...state,
    autoGpuMode: {
      mode,
...
function migrateLegacyAutoSafeModePreference(openZetcXHome, prefs, state, now) {
  ...
  writePreferences(openZetcXHome, nextPrefs);
  writeAutoGpuMode(openZetcXHome, GPU_MODE_GPU_SANDBOX_COMPAT, {
...
function resolveGpuStartupPolicy({
  ...
  if (!openZetcXHome) throw new Error("resolveGpuStartupPolicy requires openZetcXHome");

  const prefs = readPreferences(openZetcXHome);
  ...
  const migratedLegacyPolicy = platform === "win32"
    ? migrateLegacyAutoSafeModePreference(openZetcXHome, prefs, state, now)
    : null;
  ...
  if (platform === "win32" && isGpuRecoveryIncompleteStartup(state)) {
    ...
    writeAutoGpuMode(openZetcXHome, nextMode, {
...
function markGpuStartupPending({
  ...
  if (!openZetcXHome) throw new Error("markGpuStartupPending requires openZetcXHome");
  const timestamp = nowIso(now);
  const state = readState(openZetcXHome);
...
function markGpuStartupPhase({
  ...
  if (!openZetcXHome || !phase) return null;
  const state = readState(openZetcXHome);
  ...
  writeState(openZetcXHome, state);
...
function markGpuStartupReady({
  ...
  if (!openZetcXHome) throw new Error("markGpuStartupReady requires openZetcXHome");
  const state = readState(openZetcXHome);
  const timestamp = nowIso(now);
  state.startup = {
    ...(state.startup || {}),
...
function markGpuStartupFailed({
  ...
  if (!openZetcXHome) throw new Error("markGpuStartupFailed requires openZetcXHome");
  const state = readState(openZetcXHome);
  const timestamp = nowIso(now);
  state.startup = {
    ...(state.startup || {}),
...
function recordGpuChildProcessGone({
  ...
  if (!openZetcXHome || !isGpuChildProcessFailure(details)) return false;
  const timestamp = nowIso(now);
  const crash = {
...
function recordGpuInfoUpdate({
  ...
  if (!openZetcXHome || !featureStatus || typeof featureStatus !== "object") return false;
  const state = readState(openZetcXHome);
  writeState(openZetcXHome, {
    ...state,
...
function buildGpuStartupDiagnostics({ openZetcXHome, policy, app } = {}) {
  const items = [
    ``,
    `--- GPU Startup ---`,
    `Hardware acceleration preference: ${readPreferences(openZetcXHome).hardware_acceleration ?? "default"}`,
