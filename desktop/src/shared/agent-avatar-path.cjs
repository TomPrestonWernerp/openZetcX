 ...
 *   {openZetcXHome}/agents/{agentId}/avatars/agent.{png|jpg|jpeg|webp}
...
/**
 * @param {string|null|undefined} openZetcXHome - resolved HANA_HOME data root
 ...
 */
function resolveAgentAvatarPath(openZetcXHome, agentId) {
  if (typeof openZetcXHome !== "string" || openZetcXHome.length === 0) return null;
  if (!isSafeAgentId(agentId)) return null;

  const avatarDir = path.join(openZetcXHome, "agents", agentId, "avatars");
