...
let openZetcXHome = null;
try {
  const { resolveOpenZetcXHome } = require("../shared/hana-runtime-paths.cjs");
  openZetcXHome = resolveOpenZetcXHome(process.env.HANA_HOME);
  process.env.HANA_HOME = openZetcXHome;
  diagnosticsDir = path.join(openZetcXHome, "diagnostics", "desktop-launch");
} catch (err) {
  ...
  showBootstrapError(
    "openZetcX Launch Failed",
    `openZetcX failed before HANA_HOME could be resolved.\n\n${err?.message || err}\n\nDiagnostic file:\n${diagnosticPath || diagnosticsDir}`,
  );
...
function verifyWindowsInstallSurfaceBeforeMain() {
  ...
  showBootstrapError("openZetcX Launch Failed", detail);
  exitAfterBootstrapFailure();
  return false;
}
...
function loadDesktopMain() {
  try {
    ...
    showBootstrapError(
      "openZetcX Launch Failed",
      `openZetcX failed before the desktop main process finished loading.\n\n${err?.message || err}\n\nDiagnostic file:\n${diagnosticPath || diagnosticsDir}`,
    );
    exitAfterBootstrapFailure();
  }
}
