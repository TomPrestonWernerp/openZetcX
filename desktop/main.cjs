/**
 * openZetcX Desktop — Electron 主进程
 *
 * 职责：
 * 1. 创建启动窗口（splash）
 * 2. spawn() 启动 openZetcX Server
...
// onboarding/主窗口白屏且无前端报错。此处硬崩，拒绝以不可用状态启动。
{
  ...
  if (!fs.existsSync(preloadPath)) {
    ...
    try { dialog.showErrorBox("openZetcX failed to start", msg); } catch {}
  ...
  serverProcess.on("exit", async (code, signal) => {
    ...
    if (_serverRestartAttempts < 1) {
      ...
      try {
        ...
        dialog.showErrorBox("openZetcX Server", mt("dialog.serverRestartFailed", {
      ...
      dialog.showErrorBox("openZetcX Server", mt("dialog.serverMultipleCrash", {
        version: app?.getVersion?.() || "unknown",
        reason,
      }));
    }
...
function createTray() {
  ...
  const buildMenu = () => Menu.buildFromTemplate([
    { label: mt("tray.show", null, "Show openZetcX"), click: () => showPrimaryWindow() },
...
/**
 * 将崩溃日志写入 HANA_HOME/crash.log（默认 ~/.openZetcX/crash.log）并返回日志内容
 */
...
function writeCrashLog(errorMessage) {
  ...
  const content = redactMainLogText([
    `=== openZetcX Crash Log ===`,
    `openZetcX: v${app?.getVersion?.() || "unknown"}`,
    `Time: ${timestamp}`,
    `Error: ${errorMessage}`,
...
<body>
  ...
  <footer class="watermark">
    ...
    <span class="watermark-text">openZetcX</span>
  </footer>
</body>
</html>`;
}
